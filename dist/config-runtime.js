...
export function isLocalOnlyMode() {
    return process.env.GRAMATR_LOCAL_ONLY === '1';
}
/**
...
export function getContextWindowConfig() {
    let limit = getModelContextLimit(getClaudeModelFromEnv() ?? '');
    let warn_pct = 70;
    let advise_pct = 80;
    let compact_pct = 80;
    try {
        ...
        if (typeof cw?.limit === 'number' && cw.limit > 0)
            limit = cw.limit;
        if (typeof cw?.warn_pct === 'number' && cw.warn_pct > 0)
            warn_pct = cw.warn_pct;
        if (typeof cw?.advise_pct === 'number' && cw.advise_pct > 0)
            advise_pct = cw.advise_pct;
        if (typeof cw?.compact_pct === 'number' && cw.compact_pct > 0)
            compact_pct = cw.compact_pct;
    }
    catch {
        // absent or unparseable — fall through to defaults
    }
    return { limit, warn_pct, advise_pct, compact_pct };
}
