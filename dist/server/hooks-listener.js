...
async function handleRequest(req, res) {
    ...
    // ── POST /hooks/statusline — render statusline from local state ──
    // Reads token savings from local SQLite and session context from the in-process
    // session store. No remote calls — works entirely within the hook environment.
    if (url.pathname === '/hooks/statusline' && method === 'POST') {
        const raw = await readBody(req);
        try {
            const body = JSON.parse(raw);
            // Read token savings from local SQLite — no remote call.
            const { getAllTimeTokensSaved } = await import('../hooks/lib/hook-state.js');
            const allTimeSaved = getAllTimeTokensSaved();
            // Pull session-scoped call count from in-process session store if available.
            const sessionCtx = (body.session_id
                ? sessionStore.get(body.session_id)
                : sessionStore.get('__latest__')) ?? null;
            const callCount = typeof sessionCtx?.['tool_call_count'] === 'number'
                ? sessionCtx['tool_call_count']
                : 0;
            const stats = {
                token_savings_total: allTimeSaved,
                tool_call_count: callCount,
                cache_hit_rate: 0,
                server_version: undefined,
            };
            const rendered = renderStatusline(body, stats, null);
