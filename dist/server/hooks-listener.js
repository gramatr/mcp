...
/**
 * Assemble the rendered statusline string. No git info — pure grāmatr signal.
 *
 *   small:   ● grāmatr │ standard/analyze 94% │ ⚡143.5M saved (est)
 *   medium:  ● grāmatr │ standard/analyze 94% │ ⚡143.5M saved (est) │ ◇168K memories
 *   large:   ● grāmatr v0.13.31 ⬆0.13.32 │ standard/analyze 94% │ ⚡143.5M saved (est) │ ◇168K memories
 *
 * When no classification is available yet, "ready" fills the slot.
 * Segment order: grāmatr[+version] → classification/"ready" → savings (est) → obs (medium+)
 */
function renderStatusline(body, stats, classification) {
    ...
    const savingsStr = savings > 0 ? `⚡${formatTokens(savings)} saved (est)` : '';
