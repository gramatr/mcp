 ...
 *
 *   small:   ● grāmatr │ standard/analyze 94% │ ⚡143.5M saved* │ fix/auth
 *   medium:  ● grāmatr │ standard/analyze 94% │ ⚡143.5M saved* │ ◇168K memories │ gramatr · main
 *   large:   ● grāmatr v0.13.31 ⬆0.13.32 │ standard/analyze 94% │ ⚡143.5M saved* │ ◇168K memories │ main ✎2
 *
 * When no classification is available yet, "ready" fills the slot.
 * Segment order: grāmatr[+version] → classification/"ready" → savings* → obs (medium+) → git
 */
function renderStatusline(body, stats, classification) {
    ...
    // Classification segment: "standard/analyze 94%" or "ready" when no classification yet
    ...
    const upgradeStr = serverVersion && clientVersion && serverVersion !== clientVersion
        ? ` ⬆${serverVersion}`
        : '';
    // Dirty indicator: ✎N when there are modified files
    const dirtyStr = gitState?.modified && gitState.modified > 0 ? `✎${gitState.modified}` : '';
    if (size === 'small') {
        const parts = ['  ● grāmatr', classifStr];
        if (savingsStr)
    ...
    if (size === 'medium') {
        ...
        if (savingsStr)
            parts.push(savingsStr);
        if (observations > 0)
            parts.push(`◇${formatCount(observations)} memories`);
        if (gitState?.branch) {
            // project name: last segment of project_id (e.g. 'gramatr/gramatr' → 'gramatr')
            const projectName = body.project_id ? body.project_id.split('/').pop() : undefined;
            const gitSeg = projectName ? `${projectName} · ${gitState.branch}` : gitState.branch;
            parts.push(gitSeg);
        }
        return parts.join(' │ ') + '\n';
    }
    // large — version + upgrade indicator on the grāmatr pill
    const versionStr = clientVersion ? `grāmatr v${clientVersion}${upgradeStr}` : 'grāmatr';
    const parts = [`  ● ${versionStr}`, classifStr];
    if (savingsStr)
        parts.push(savingsStr);
    if (observations > 0)
        parts.push(`◇${formatCount(observations)} memories`);
    if (gitState?.branch) {
        const gitSeg = dirtyStr ? `${gitState.branch} ${dirtyStr}` : gitState.branch;
        parts.push(gitSeg);
    }
    return parts.join(' │ ') + '\n';
}
...
async function handleRequest(req, res) {
    ...
    if (url.pathname === '/hooks/statusline' && method === 'POST') {
        ...
        try {
            ...
            // Fall back to 0 on any failure — local SQLite is a different counter and disagrees
            // with the server Redis value (fix: #1807).
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    // Token expired or missing — surface re-auth badge to the user
                    const rendered = '  ⚠ grāmatr [re-login required] — run /mcp or `gramatr login`\n';
                    json(res, 200, { rendered });
                    return;
                }
            }
            catch {
                // Non-critical — statusline shows 0 when server is unreachable
            }
...
export async function startHooksListener() {
    ...
    return new Promise((resolve, reject) => {
        ...
        _server.listen(0, '127.0.0.1', () => {
            ...
            try {
                // Write PORT:PID so statusline can validate the process is still alive
                // before attempting a connection — avoids 2s timeout on stale port files.
                writeFileSync(getPortFilePath(), `${port}:${process.pid}`, { mode: 0o600 });
            }
            catch { /* non-critical */ }
            resolve(port);
        });
