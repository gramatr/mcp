...
export async function startServer() {
    ...
    if (!token) {
        process.stderr.write('[gramatr-mcp] No auth token found. Run: gramatr login\n');
    }
    ...
    if (isLocalOnlyMode()) {
        ...
        process.stderr.write(`[gramatr-mcp] Local-only mode (GRAMATR_LOCAL_ONLY=1): exposing ${localCount} local tools only\n`);
    }
    else {
        ...
        if (currentUser) {
            ...
            setCurrentMode(mode);
            process.stderr.write(`[gramatr-mcp] privilege mode: ${mode} (${currentUser.actor_role})\n`);
        }
    }
    // Load remote prompts
    try {
