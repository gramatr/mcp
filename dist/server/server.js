...
import { fetchRemotePrompt, fetchRemoteResource, fetchCurrentUser } from '../proxy/remote-client.js';
import { getSessionId, getProjectId, getClientSessionId } from '../intelligence/session-manager.js';
...
import { isLocalOnlyMode } from '../config-runtime.js';
import { getCurrentMode, filterToolsByMode, setCurrentMode, privilegeModeFromEntitlement } from '../proxy/tool-privilege.js';
const SERVER_NAME = 'gramatr';
/**
 * Create and configure the MCP server.
 */
export function createServer() {
    const server = new Server({ name: SERVER_NAME, version: VERSION }, { capabilities: { tools: {}, prompts: {}, resources: {} } });
    // ── tools/list handler — remote + local tools (or local-only when GRAMATR_LOCAL_ONLY=1) ──
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        const localTools = getLocalToolDefinitions();
        // In local-only mode (cloud addon active) expose only local_* daemon tools.
        // All server tools are already served by the cloud addon — no duplication.
        const combined = isLocalOnlyMode()
            ? localTools
            : [...getTools(), ...localTools];
        // Deduplicate by tool name — remote definitions come first so local ones shadow them
        // when names collide (e.g. local_config and local_fetch may appear in both the remote
        // manifest and the local tool list when the remote server also exposes them). (#1701)
        const seen = new Map();
        for (const t of combined) {
            if (!seen.has(t.name))
                seen.set(t.name, t);
        }
        // Apply privilege mode filter — local tools (local_* prefix) always pass through;
        // remote tools are filtered by the session's current privilege mode. (#1705)
        const mode = getCurrentMode();
        const filtered = filterToolsByMode(Array.from(seen.values()), mode);
        return {
    ...
    // Pass the active session context so session-scoped resources
    // (gramatr://session/context, /statusline/*) resolve precisely for THIS
    // window via the `X-Gramatr-Client-Session` header (v0.20.25 PR #2775).
...
export async function startServer() {
    ...
    if (!token) {
        process.stderr.write('[gramatr] No auth token found. Run: gramatr login\n');
    }
    // In local-only mode, skip remote tool loading entirely —
    // only local_* daemon tools are served; the cloud addon provides the rest.
    if (isLocalOnlyMode()) {
        const localCount = getLocalToolDefinitions().length;
        process.stderr.write(`[gramatr] Local-only mode (GRAMATR_LOCAL_ONLY=1): exposing ${localCount} local tools only\n`);
    }
    else {
        ...
        const localCount = getLocalToolDefinitions().length;
        process.stderr.write(`[gramatr] Loaded ${tools.length} remote + ${localCount} local tools\n`);
        // Derive privilege mode from subscription entitlement. (#1705)
        // fetchCurrentUser() returns null if server is unreachable or auth is absent —
        // leave mode at default 'user' in that case, never crash.
        const currentUser = await fetchCurrentUser();
        if (currentUser) {
            const mode = privilegeModeFromEntitlement(currentUser.actor_role, currentUser.entitlement_level);
            setCurrentMode(mode);
            process.stderr.write(`[gramatr] privilege mode: ${mode} (${currentUser.actor_role})\n`);
        }
    }
    // Load remote prompts
    try {
        ...
        if (prompts.length > 0) {
            process.stderr.write(`[gramatr] Loaded ${prompts.length} remote prompts\n`);
        }
    }
    catch {
    ...
    try {
        ...
        if (resources.length > 0) {
            process.stderr.write(`[gramatr] Loaded ${resources.length} remote resources\n`);
        }
    }
    catch {
    ...
    if (pending > 0) {
        process.stderr.write(`[gramatr] Found ${pending} queued call(s), replaying...\n`);
        void replayQueue();
    }
    ...
    process.stderr.write(`[gramatr] Server started (stdio transport)\n`);
    ...
    try {
        ...
        process.stderr.write(`[gramatr] Hooks listener on port ${hooksPort}\n`);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[gramatr] Warning: hooks listener failed to start (${msg})\n`);
    }
    ...
    process.stdin.on('end', () => {
        process.stderr.write('[gramatr] stdin closed — parent disconnected, exiting\n');
        stopHooksListener();
        process.exit(0);
    });
    ...
    let idleTimer = setTimeout(() => {
        process.stderr.write('[gramatr] idle timeout (2h) — exiting\n');
        stopHooksListener();
        process.exit(0);
    }, IDLE_TIMEOUT_MS);
    idleTimer.unref();
