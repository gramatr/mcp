...
export type DaemonMethod = 'tool/call' | 'session/register' | 'session/release' | 'session/context/get' | 'session/context/set' | 'project/resolve' | 'project/cache-set' | 'db/query' | 'agent/store' | 'agent/get' | 'agent/list' | 'agent/expire' | 'daemon/ping' | 'daemon/shutdown' | 'orchestration/run-create' | 'orchestration/run-get' | 'orchestration/run-list' | 'orchestration/run-update' | 'orchestration/task-enqueue' | 'orchestration/task-get' | 'orchestration/task-list' | 'orchestration/task-pickup' | 'orchestration/task-complete' | 'orchestration/approval-create' | 'orchestration/approval-resolve' | 'orchestration/approval-get' | 'orchestration/status-dashboard' | 'db/write';
/**
 * Parameters for the db/write IPC method.
 *
 * Routes INSERT/INSERT OR REPLACE operations for turns, op_history, and
 * compacts through the daemon's owned SQLite connection — eliminating
 * SQLITE_BUSY collisions when 4+ concurrent hook processes write simultaneously.
 */
export interface DbWriteParams {
    table: 'turns' | 'op_history' | 'compacts';
    operation: 'insert' | 'insert_or_replace';
