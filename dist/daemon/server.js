...
import { logDaemon } from './daemon-logger.js';
...
export async function dispatchRpcRequest(req) {
    ...
    switch (method) {
        case 'tool/call': {
            ...
            // Resolve gramatr session context from the hook's Claude session ID.
            // Hooks pass session_id (their CLAUDE_SESSION_ID) so we can look up the
            // interaction_id and project_id written by the session-start hook.
            let sessionCtx;
            const hookSessionId = params.session_id;
            if (hookSessionId) {
                const ctxEntry = contextStore.get(hookSessionId);
                if (ctxEntry) {
                    const interactionId = ctxEntry['interaction_id'];
                    const projectId = ctxEntry['project_id'];
                    if (interactionId || projectId) {
                        sessionCtx = {
                            sessionId: interactionId ?? hookSessionId,
                            projectId: projectId ?? undefined,
                        };
                    }
                }
                else {
                    // Not in memory — try SQLite (survives daemon restart)
                    const db = sqliteOwner.getDb();
                    if (db) {
                        try {
                            const row = db
                                .prepare('SELECT interaction_id, project_id FROM session_context WHERE session_id = ? ORDER BY updated_at DESC LIMIT 1')
                                .get(hookSessionId);
                            if (row) {
                                sessionCtx = {
                                    sessionId: row['interaction_id'] ?? hookSessionId,
            ...
            return callRemoteTool(name, args, sessionCtx);
        }
        ...
        case 'db/write': {
            const { table, operation, record } = params;
            const db = sqliteOwner.getDb();
            if (!db)
            ...
            try {
                switch (table) {
                    case 'turns': {
                        const sql = operation === 'insert_or_replace'
                 ...
                 VALUES
                  (@session_id, @client_session_id, @project_id, @agent_name,
                   @turn_number, @timestamp, @prompt, @effort_level, @intent_type, @confidence, @tokens_saved)`;
                    ...
                    case 'op_history': {
                        const sql = operation === 'insert_or_replace'
                            ? `INSERT OR REPLACE INTO op_history (session_id, tool, time_ms, tokens_saved, timestamp)
                 ...
                 VALUES (@session_id, @tool, @time_ms, @tokens_saved, @timestamp)`;
                    ...
                    case 'compacts': {
                        // compacts always uses INSERT OR REPLACE — only one compact per id.
                        db.prepare(`
            ...
            `).run(record);
                    ...
                    default: {
                        throw Object.assign(new Error(`db/write: unknown table: ${String(table)}`), { code: -32601 });
                    }
                }
            }
            catch (err) {
                if (err !== null && typeof err === 'object' && 'code' in err)
                    throw err;
                throw Object.assign(new Error(`db/write(${String(table)}) failed: ${err instanceof Error ? err.message : String(err)}`), { code: -32603 });
            }
            sqliteOwner.resetIdleTimer();
            return { ok: true };
        }
...
async function handleRpc(line, socket, rl) {
    ...
    try {
        ...
        // Log internal errors (code -32603) structurally — they indicate unexpected
        // failures, not intentional JSON-RPC error responses.
        if (code === -32603) {
            logDaemon('ERROR', 'unhandled error in request handler', {
                method: req.method,
                id: req.id,
                error: message,
...
export function createDaemonServer(authToken) {
    ...
    _server = server;
    server.on('error', (err) => {
        logDaemon('ERROR', 'IPC server error', { code: err.code, message: err.message });
