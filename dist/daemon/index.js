...
import { redirectStderrToLog, registerCrashHandlers, logDaemon } from './daemon-logger.js';
...
export async function startDaemon() {
    // Install crash handlers and stderr→logfile mirroring as the very first
    // thing — before any I/O that could fail.  This ensures that even startup
    // errors are captured in ~/.gramatr/debug/daemon.log.
    redirectStderrToLog();
...
async function gracefulShutdown(server, reason) {
    // Signal the crash handler that this is intentional so no crash record is written.
    const markGraceful = globalThis['__gramatrDaemonGracefulExit'];
    if (typeof markGraceful === 'function')
        markGraceful();
    logDaemon('INFO', `daemon shutting down (${reason})`);
