export const MCP_HOOK_BIN_RELATIVE = 'mcp/dist/bin/gramatr-mcp.js';
...
const CLAUDE_HOOKS = [
    ...
    { event: 'SessionStart', mcpHookName: 'session-start' },
    { event: 'SessionEnd', mcpHookName: 'session-end' },
    { event: 'Stop', mcpHookName: 'stop' },
];
const CODEX_HOOKS = [
    {
        event: 'SessionStart',
...
function buildHookCommand(clientDir, spec) {
    if (spec.mcpHookName) {
        return `node "${clientDir}/${MCP_HOOK_BIN_RELATIVE}" hook ${spec.mcpHookName}`;
    }
