...
Before responding to any user message, call the route_request tool with
...
The response is a gmtr.intelligence.contract.v2 packet containing classification,
behavioral directives, phase templates, quality gates, and enrichment data.
...
If a tool response includes schema "gmtr.tool.result.v1", consume the
payload from its data field.

Without calling route_request, your response will be generic and miss
...
load_handoff before answering, then search_semantic for detail.

After completing non-trivial work, you MUST call classification_feedback
(trains the classifier) and save_reflection (feeds the learning pipeline).
