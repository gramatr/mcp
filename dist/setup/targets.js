...
export function buildLocalMcpServerEntry(home, serverUrl) {
    return {
        command: 'node',
        args: [join(home, '.gramatr', 'mcp', 'dist', 'bin', 'gramatr-mcp.js')],
        env: {
...
export function buildGeminiHooksFile(home) {
    const hookBase = `node "${join(home, '.gramatr', 'mcp', 'dist', 'bin', 'gramatr-mcp.js')}" hook`;
    return {
        hooks: {
            SessionStart: [
                {
