...
async function handleSetMode(args) {
    ...
    process.stderr.write(`[gramatr] privilege mode elevated: ${previous} → ${requested} — ${reason}\n`);
    return {
        content: [
            {
                type: "text",
...
async function handleLocalFetchPacket(args) {
    ...
    try {
        ...
        if (record) {
            ...
            // Defense-in-depth: strip pipeline-internal fields from cached packets that
            // pre-date the server-side fix in assemble-packet.ts (#1879).
            if (packet !== null && typeof packet === "object") {
                const p = packet;
                if (p.classification !== null && typeof p.classification === "object") {
                    const c = p.classification;
                    delete c._sessionContext;
                    delete c._curatedContext;
                }
            }
            return {
                content: [
