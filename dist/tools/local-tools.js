...
const LOCAL_TOOL_NAMES = new Set([
    'gramatr_local_status',
    'gramatr_local_clear_cache',
    'gramatr_local_config',
    'gramatr_local_metrics',
    'gramatr_resolve_project',
]);
/**
 * Check if a tool name is handled locally (fully or local-first).
 */
export function isLocalTool(name) {
    return name.startsWith('gramatr_local_') || LOCAL_TOOL_NAMES.has(name);
}
/**
...
async function handleResolveProject(args) {
    ...
    try {
        const result = await callRemoteTool('gramatr_resolve_project', args);
        return result;
    }
    catch (err) {
        return {
