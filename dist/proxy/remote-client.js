 ...
 */
export async function callRemoteTool(toolName, args, sessionContext) {
    const payload = {
        jsonrpc: '2.0',
        id: ++requestId,
        method: 'tools/call',
 ...
 */
export async function fetchRemoteResource(uri, sessionContext) {
    const payload = {
        jsonrpc: '2.0',
        id: ++requestId,
...
export async function fetchCurrentUser() {
    ...
    if (!token)
        return null;
    // Strip /mcp suffix to get the base REST URL
    ...
    try {
        ...
        if (body !== null &&
            ...
            'user_id' in body &&
            'actor_role' in body &&
            'entitlement_level' in body) {
            return body;
        }
        return null;
    }
    catch {
        // Network failure, timeout, parse error — degrade cleanly
        return null;
    }
}
/**
...
async function postToRemote(payload, sessionContext) {
    ...
    // Per-window correlation handle for the session/context resource handler.
    // v0.20.25 PR #2775 — disambiguates concurrent Claude Code windows that
    // share a user_id; without this header the server falls back to the
    // last-write-wins user-keyed pointer.
