...
// ── Request ID counter ──
let _requestId = 0;
let _lastCallUsedDaemon = true;
/** Returns true if the most recent callTool() was served by the local daemon (socket or HTTP). */
export function wasDaemonAvailable() {
...
// ── Daemon auto-restart ───────────────────────────────────────────────────────
let _restartAttempted = false; // one attempt per hook process lifetime
/**
 * Attempt to locate and spawn the gramatr daemon binary.
 * Fallback chain: ~/.gramatr.json gramatr_binary → `which gramatr` → `npx @gramatr/mcp`
 * Returns true if spawn succeeded (process started, not necessarily ready).
 */
function spawnDaemon() {
    try {
        // 1. Read gramatr_binary from ~/.gramatr.json
        let binaryPath;
        try {
            const home = process.env['HOME'] ?? process.env['USERPROFILE'] ?? '';
            const configPath = joinPath(home, '.gramatr.json');
            const config = JSON.parse(readFileSync(configPath, 'utf8'));
            if (typeof config['gramatr_binary'] === 'string' && config['gramatr_binary']) {
                binaryPath = config['gramatr_binary'];
            }
        }
        catch { /* missing or malformed — try fallback */ }
        // 2. which gramatr
        if (!binaryPath) {
            try {
                const result = spawnSync(process.platform === 'win32' ? 'where' : 'which', ['gramatr'], { encoding: 'utf8', timeout: 1000 });
        ...
        // 3. npx @gramatr/mcp as last resort — skip if no stable path found
        // (npx spawns a temp binary; we only use it if the above both fail)
        const [cmd, ...args] = binaryPath
            ? [binaryPath, 'daemon', 'start']
            : ['npx', '--yes', '@gramatr/mcp', 'daemon', 'start'];
...
/**
 * Poll the daemon socket until it appears or we time out.
 */
...
/**
 * One-shot daemon restart attempt for this hook process.
 * Returns true if the daemon appears ready after restart.
 */
async function tryRestartDaemon() {
    ...
    if (!spawned)
        return false;
    return waitForDaemonSocket(3000);
}
// ── Public API ────────────────────────────────────────────────────────────────
/**
...
export async function callTool(name, args, hookSessionId) {
    ...
    // Auto-restart: if both socket and HTTP are dead, try to restart the daemon once
...
/**
 * Route a SQLite write (INSERT / INSERT OR REPLACE) through the daemon broker.
 *
 * When 4+ concurrent sessions write directly to the SQLite file they collide
 * with SQLITE_BUSY. Routing through the daemon's single connection serialises
 * all writes without contention.
 *
 * Falls back gracefully when the daemon is unavailable — callers should
 * fall back to direct write on `false` return.
 */
export async function dbWriteViaDaemon(table, operation, record) {
