...
export interface CompactConfig {
    /** How many turns between compact checks (default 15). */
    every_turns: number;
    /**
     * When true: compact fires silently in the background — no user-facing injection.
     * When false (default): compact does NOT fire automatically; user receives
     * escalating reminder hints until they act.
     */
    auto: boolean;
    /** When auto=false, how often to re-remind after the first threshold (default 5). */
    remind_every: number;
}
/**
 ...
 */
export declare function getCompactConfig(): CompactConfig;
...
/**
 * Returns the context window token limit for a known Claude model.
 * Claude Opus 4.7 has a 1M token context window; all other current Claude models have 200K.
...
/** Returns true when the local MCP proxy should expose only local_* daemon tools (cloud plugin active). */
export declare function isLocalOnlyMode(): boolean;
export interface ContextWindowConfig {
    /** Total context window token limit (default 200_000). */
    limit: number;
    /** Percentage at which to inject an amber warning hint (default 70). */
    warn_pct: number;
    /** Percentage at which to inject a red/urgent MANDATORY hint (default 80). */
    advise_pct: number;
    /** Percentage at which to trigger auto-compact when auto=true (default 80). */
    compact_pct: number;
}
/**
 * Context window config. Priority:
 *   1. ~/.gramatr.json context_window.* (canonical)
 *   2. Defaults: limit=200_000, warn_pct=70, advise_pct=80, compact_pct=80
 */
export declare function getContextWindowConfig(): ContextWindowConfig;
