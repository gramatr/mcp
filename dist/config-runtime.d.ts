...
export interface ContextWindowConfig {
    /** Total context window token limit (default 180_000). */
    limit: number;
    /** Percentage at which to inject an amber warning hint (default 75). */
    warn_pct: number;
    /** Percentage at which to inject a red/urgent MANDATORY hint (default 80). */
    advise_pct: number;
    /** Percentage at which to trigger auto-compact when auto=true (default 90). */
    compact_pct: number;
}
/**
 * Context window config. Priority:
 *   1. ~/.gramatr.json context_window.* (canonical)
 *   2. Defaults: limit=180_000, warn_pct=75, advise_pct=80, compact_pct=90
 */
