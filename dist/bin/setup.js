...
export function setupClaudeCode(dryRun = false, cleanInstall = false, showPrompts = false, showStatusLine = true) {
    if (!dryRun) {
        killStaleMcpServers();
    }
    migrateSettingsJson();
...
export function setupClaude(dryRun = false, cleanInstall = false, showPrompts = false, showStatusLine = true) {
    return setupClaudeCode(dryRun, cleanInstall, showPrompts, showStatusLine);
}
...
export function setupAutoInstall(options = {}) {
    ...
    for (const target of selected) {
        switch (target) {
            case "claude-code":
                setupClaudeCode(dryRun, false, showPrompts);
                break;
