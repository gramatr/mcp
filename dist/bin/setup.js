...
export function setupClaude(dryRun = false, cleanInstall = false, showPrompts = false, showStatusLine = true) {
    ...
    const statusLineCommand = mcpCommand === 'gramatr-mcp'
        ? 'gramatr-mcp statusline'
    ...
    const existingMcpServers = settingsWithoutGramatrKeys.mcpServers ?? {};
    const { gramatr: _gramatrMcpServer, ...mcpServersWithoutGramatr } = existingMcpServers;
    ...
    const mergedSettings = addPluginRegistration({
        ...
        env: (() => {
            ...
            const { GRAMATR_TOKEN: _t, AIOS_MCP_TOKEN: _lt, GRAMATR_URL: _u, ...existingEnv } = settingsWithoutGramatrKeys.env ?? {};
            return {
                ...existingEnv,
