...
// Both delegate to `node $GRAMATR_DIR/mcp/dist/bin/gramatr-mcp.js hook ...`
// so hooks never spawn npx — they route through the already-running daemon process.
function writeHookWrappers(gramatrDir) {
    const binDir = join(gramatrDir, 'bin');
    mkdirSync(binDir, { recursive: true });
    const shWrapper = join(binDir, 'gramatr-hook');
    writeFileSync(shWrapper, '#!/bin/sh\nexec node "${GRAMATR_DIR:-$HOME/.gramatr}/mcp/dist/bin/gramatr-mcp.js" hook "$@"\n', { mode: 0o755 });
