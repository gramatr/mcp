 ...
 *   gramatr setup claude-code     Auto-configure Claude Code settings and hooks
 *   gramatr setup claude          Alias for claude-code (legacy)
 *   gramatr setup claude-code --clean-install  Remove legacy Claude/PAI/Fabric artifacts first
 *   gramatr setup codex           Auto-configure Codex hook settings
 *   gramatr setup opencode        Auto-configure OpenCode MCP settings
...
export function printHelp() {
  ...
  Usage:
    gramatr install                Idempotent Claude Code install (hook + CLAUDE.md merge);
                                      sweeps stale grāmatr hook entries by default (v0.20.6+)
    gramatr install --keep-legacy  Preserve stale grāmatr-tagged hook entries instead of removing them
    gramatr install --pre-auth=<tier>
                                      Pre-authorize grāmatr MCP tools; tier ∈ {none|read|all}
    gramatr install --non-interactive
                                      Skip prompts (CI-friendly); pre-auth defaults to none unless set
    gramatr install --dry-run      Show actions without writing
    gramatr uninstall              Remove gramatr hook + CLAUDE.md section
    gramatr uninstall --purge      Also remove ~/.gramatr.json token/config
    ...
    gramatr logout                 Remove stored credentials
    gramatr clear-creds            Remove all stored local credentials
    gramatr setup claude-code      Configure Claude Code to use this server
    gramatr setup claude           Alias for claude-code (legacy)
    gramatr setup claude-code --clean-install
                                      Remove legacy pre-rebrand artifacts first
    gramatr setup codex            Configure Codex hook wiring
    gramatr setup opencode         Configure OpenCode MCP settings
  ...
  Setup:
    gramatr setup claude-code        Write MCP config to ~/.claude.json
    gramatr setup claude             Alias for claude-code (legacy)
    gramatr setup opencode           Write OpenCode MCP config
    gramatr setup claude-code --clean-install
...
export async function cliMain(cliArgs = process.argv.slice(2)) {
    ...
    if (cliArgs[0] === "install") {
        const { runInstallCli } = await import("./install.js");
        process.exit(await runInstallCli(cliArgs.slice(1)));
    }
    if (cliArgs[0] === "uninstall") {
        const { runUninstallCli } = await import("./install.js");
        process.exit(await runUninstallCli(cliArgs.slice(1)));
    }
    ...
    if (cliArgs[0] === "setup") {
        ...
        // Proactive notice: Node emits an ExperimentalWarning when node:sqlite is first
        // imported (via setup.js → gramatr-hook-utils → hook-state). Print this before any
        // heavyweight import so users see the explanation before they see the warning. (#1694)
        if (target !== "verify") {
            process.stderr.write("[gramatr-mcp] Note: you may see a Node.js SQLite ExperimentalWarning below.\n" +
                "              This is expected — gramatr uses Node's built-in SQLite for local\n" +
                "              credential caching. It is not an error and can be safely ignored.\n");
        }
        if (target === "claude-code" || target === "claude") {
            ...
            // Note: "claude" is a legacy alias for "claude-code".
            ...
            const { setupClaudeCode } = await import("./setup.js");
            setupClaudeCode(dryRun, cleanInstall, showPrompts, showStatusLine);
            return;
        }
        if (target === "codex") {
            if (cleanInstall) {
        ...
        if (target === "opencode") {
            ...
            const { setupOpenCode } = await import("./setup.js");
        ...
        process.stderr.write("  Supported: claude-code | claude (alias) | codex | claude-desktop | chatgpt-desktop | gemini | cursor | windsurf | vscode | all | auto | web | clean-install | verify\n");
        process.exit(1);
    }
    ...
    try {
        ...
        if (!marketplaceRegistered || !existsSync(pluginJsonPath)) {
            ...
            const { setupClaudeCode } = await import("./setup.js");
            setupClaudeCode(false, false, false);
        }
    }
    catch {
