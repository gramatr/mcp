...
export function printHelp() {
  ...
  Usage:
    ...
    gramatr-mcp daemon stop            Send shutdown request to running daemon
    gramatr-mcp daemon status          Print daemon liveness and session count
    gramatr-mcp --mode <level>         Set initial privilege mode (user|team|org|admin)
    gramatr-mcp --version              Print version
...
export async function cliMain(cliArgs = process.argv.slice(2)) {
    ...
    // Sets the session's privilege mode at startup. Session-scoped only — resets on restart.
    {
        ...
        if (modeIdx !== -1) {
            ...
            if (!modeValue || !isValidMode(modeValue)) {
                ...
                process.stderr.write(`[gramatr-mcp] Invalid --mode value: "${String(modeValue)}". Must be one of: ${valid}\n`);
                process.exit(1);
                return;
            }
            setCurrentMode(modeValue);
            process.stderr.write(`[gramatr-mcp] Privilege mode set to: ${modeValue}\n`);
        }
    }
    ...
    if (cliArgs[0] && !cliArgs[0].startsWith('-')) {
        ...
        process.stderr.write(`[gramatr-mcp] Unknown subcommand: ${typed}\n`);
        ...
        if (suggestion) {
            process.stderr.write(`  Did you mean: gramatr-mcp ${suggestion.cmd}?\n`);
        }
        process.stderr.write('  Run: gramatr-mcp --help\n');
        process.exit(1);
    }
