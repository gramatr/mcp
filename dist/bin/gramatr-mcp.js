...
export function printHelp() {
  ...
  Usage:
    ...
    gramatr-mcp setup clean-install    Remove stale legacy install artifacts globally
  ...
  Setup:
    ...
    gramatr-mcp setup auto               Detect and install supported local clients
    gramatr-mcp setup clean-install      Remove stale legacy install artifacts first
...
export async function cliMain(cliArgs = process.argv.slice(2)) {
    ...
    if (cliArgs[0] === 'setup') {
        ...
        if (target === 'claude') {
            ...
            setupClaude(dryRun, cleanInstall);
            return;
        }
        if (target === 'codex') {
            if (cleanInstall) {
        ...
        process.stderr.write('  Supported: claude | codex | claude-desktop | chatgpt-desktop | gemini | cursor | windsurf | vscode | auto | web | clean-install | verify\n');
        process.exit(1);
    }
