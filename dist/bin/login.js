...
import { clearStaleClaudeJsonAuthHeader, getClaudeJsonSyncState, writeClaudeJsonAuthHeader, } from "./setup-config-io.js";
// ── Config ──
...
const BRAND_CSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    --gramatr-bg: #0E0E0E;
    --gramatr-surface: #1A1A1A;
    --gramatr-border: #333333;
    --gramatr-text: #ECECEC;
    --gramatr-text-secondary: #A0A0A0;
    --gramatr-text-muted: #707070;
    --gramatr-text-faint: #4A4A4A;
    --gramatr-primary: #3B82F6;
    --gramatr-accent: #60A5FA;
    --gramatr-success: #4ADE80;
    --gramatr-error: #F87171;
  ...
  body {
    background: var(--gramatr-bg);
    color: var(--gramatr-text);
  ...
  .card {
    background: var(--gramatr-surface);
    border: 1px solid var(--gramatr-border);
    border-radius: 12px;
    padding: 2.5rem;
    max-width: 480px;
    width: 100%;
  ...
  p {
    color: var(--gramatr-text-secondary);
    font-size: 0.9375rem;
    line-height: 1.6;
    margin: 0;
  }
  .hint {
    margin-top: 1.75rem;
    padding-top: 1.25rem;
    border-top: 1px solid var(--gramatr-border);
    font-size: 0.8125rem;
    color: var(--gramatr-text-muted);
  }
  .hint code {
...
// ── Headless Detection ──
//
// #2372 P2-B: detection logic mirrors the canonical helper in
// `@gramatr/gateway` (`packages/gateway/src/auth/clients/client-adapter.ts`).
// This package cannot import gateway at runtime (it must work standalone in
// the published `@gramatr/mcp` npm package without the workspace siblings),
// so the heuristic is duplicated here intentionally and kept in sync via the
// unit tests in `login.test.ts`. Update both call sites together.
export function isHeadless(forceFlag = false) {
    // Explicit override — user passed --headless / --device-code,
    // OR env GRAMATR_LOGIN_HEADLESS=1, OR env GRAMATR_HEADLESS=true
    if (forceFlag)
        return true;
    if (process.env.GRAMATR_LOGIN_HEADLESS === "1")
        return true;
    if (process.env.GRAMATR_HEADLESS === "true" || process.env.GRAMATR_HEADLESS === "1")
        return true;
    ...
    if (process.env.SSH_CLIENT || process.env.SSH_CONNECTION || process.env.SSH_TTY)
        return true;
    // WSL interop into Linux
    if (process.env.WSL_INTEROP || process.env.WSL_DISTRO_NAME)
        return true;
    // Devcontainer
    if (process.env.REMOTE_CONTAINERS || process.env.DEVCONTAINER)
        return true;
    // GitHub Codespaces
...
/**
 * #2455 — Bridge the new access token into Claude Code's `~/.claude.json` so
 * Claude Code's built-in MCP HTTP client picks it up immediately. Called from
 * every success path in login (device flow, browser flow, token flow). Warns
 * and continues if the file is owned by root or otherwise unwritable — the
 * canonical config in `~/.gramatr.json` is already written by the caller.
 */
function bridgeTokenToClaudeJson(token) {
    try {
        const ok = writeClaudeJsonAuthHeader(token, { url: DEFAULT_SERVER });
        if (ok) {
            console.log("  ✓ Token bridged to ~/.claude.json for Claude Code MCP");
        }
        else {
            console.log("  ⚠ Could not write ~/.claude.json (permissions?) — Claude Code may not pick up the token automatically");
        }
    }
    catch (e) {
        console.log(`  ⚠ ~/.claude.json bridge skipped: ${e.message}`);
    }
}
/**
...
export async function showStatus() {
    ...
    // #2453 — surface stale Bearer header in ~/.claude.json. The user may
    // have a legacy HTTP-transport mcpServers.gramatr entry that holds a
    // token gramatr no longer recognises; that header keeps Claude Code in
    // a dead OAuth loop until cleared.
    try {
        const claudeJsonPath = join(HOME, ".claude.json");
        const claudeRaw = JSON.parse(readFileSync(claudeJsonPath, "utf8"));
        const claudeHeader = claudeRaw?.mcpServers?.gramatr?.headers?.Authorization ??
            claudeRaw?.mcpServers?.gramatr?.headers?.authorization;
        if (typeof claudeHeader === "string" && claudeHeader.length > 0) {
            console.log("  Note:    ~/.claude.json has an HTTP-transport Bearer header.");
            console.log("           If you hit an OAuth loop, run: gramatr logout && gramatr login");
        }
    }
    catch {
        // Non-fatal — no claude.json or unreadable
    }
    // #2455 — report whether ~/.claude.json and ~/.gramatr.json are in sync.
    try {
        const sync = getClaudeJsonSyncState();
        switch (sync) {
            case "synced":
                console.log("  Sync:    ✓ ~/.claude.json and ~/.gramatr.json hold the same token");
                break;
            case "diverged":
                console.log("  Sync:    ⚠ tokens DIVERGE between ~/.claude.json and ~/.gramatr.json");
                console.log("           One of the files was manually edited. Run `gramatr login` to resync.");
                break;
            case "missing-claude":
                console.log("  Sync:    ⚠ ~/.claude.json has no Bearer header for gramatr (Claude Code may not pick up the token).");
                console.log("           Run `gramatr login` to bridge it.");
                break;
            case "missing-gramatr":
                console.log("  Sync:    ⚠ ~/.gramatr.json has no token but ~/.claude.json does — inconsistent state");
                break;
            case "unreadable":
                console.log("  Sync:    ⚠ one of the config files is unreadable / malformed");
                break;
            case "neither":
                // No tokens anywhere — already covered by the missing-token branch above.
                break;
        }
    }
    catch {
...
export async function logout() {
    ...
    writeConfig(config);
    // #2453 — also strip any stale Bearer header from ~/.claude.json so the
    // next Claude Code session does not present a token gramatr no longer
    // believes is valid.
    const cleared = clearStaleClaudeJsonAuthHeader();
    console.log("\n  ✓ Logged out. Token removed from ~/.gramatr.json");
    if (cleared) {
        console.log("  ✓ Stale Bearer header removed from ~/.claude.json");
    }
    console.log("");
}
...
export async function loginBrowser(opts = {}) {
    ...
    // #2453 — strip any stale Bearer token from ~/.claude.json BEFORE starting
    // OAuth. If a legacy HTTP-transport `mcpServers.gramatr` entry has a token
    // the server now rejects, leaving it in place confuses the OAuth bridge
    // (the stale Authorization header travels alongside the OAuth dance and
    // Claude Code's MCP client gets stuck in a re-auth loop). Clear it now so
    // the new token issued below replaces a clean slate.
    try {
        if (clearStaleClaudeJsonAuthHeader()) {
            console.log("  Cleared stale Bearer header from ~/.claude.json");
        }
    }
    catch {
        // Non-fatal — proceed with OAuth even if cleanup fails.
    }
    ...
    console.log("");
    // Determine whether to use the RFC 8628 device flow or the PKCE browser
    // flow.  Decision tree:
    //
    //   1. --device-code / --headless flag set → device flow, no prompt.
    //   2. stdin is a TTY → always offer an interactive choice (removes
    //      dependence on fragile env-var detection that silently picks the
    //      wrong path on e.g. Linux with DISPLAY set but no browser, or fresh
    //      WSL installs without WSL_INTEROP populated).
    //   3. Non-interactive stdin → fall back to isHeadless() env-var detection
    //      (CI, piped scripts, Docker, etc.).
    let useDeviceFlow;
    if (opts.forceHeadless) {
        console.log("  Device code flow forced (--device-code).\n");
        useDeviceFlow = true;
    }
    else if (process.stdin.isTTY) {
        // Always offer the choice when the terminal is interactive.
        const { createInterface } = await import("readline/promises");
        const rl = createInterface({ input: process.stdin, output: process.stdout });
        try {
            process.stdout.write("  How would you like to authenticate?\n" +
                "    1) Browser        (opens a browser window)     ← default\n" +
                "    2) Device code    (for headless / remote / WSL)\n\n" +
                "  Enter 1 or 2 [1]: ");
            const answer = await rl.question("");
            useDeviceFlow = answer.trim() === "2";
        }
        finally {
            rl.close();
        }
        if (useDeviceFlow) {
            console.log("  Using device code flow.\n");
        }
    }
    else {
        // Non-interactive stdin — fall back to env-var detection.
        useDeviceFlow = isHeadless();
        if (useDeviceFlow) {
            console.log("  Headless environment detected — switching to device code flow.\n");
        }
    }
    if (useDeviceFlow) {
        try {
            ...
            writeConfig(config);
            bridgeTokenToClaudeJson(accessToken);
            killStaleMcpServers();
            await runPostLoginOnboarding();
            console.log("");
...
export async function main(rawArgs = process.argv.slice(2)) {
  ...
  Usage:
    ...
    gramatr login --device-code      Force the RFC 8628 device flow regardless of context
                                         (alias: --force-device-code, --headless)
  ...
  Auto-detected headless contexts (auto-switch to device flow):
    SSH (SSH_CLIENT / SSH_TTY / SSH_CONNECTION)
    WSL (WSL_INTEROP / WSL_DISTRO_NAME)
    Devcontainers (REMOTE_CONTAINERS / DEVCONTAINER)
    GitHub Codespaces (CODESPACES)
    GitPod (GITPOD_WORKSPACE_URL)
    Linux without DISPLAY / WAYLAND_DISPLAY

  Environment:
    GRAMATR_LOGIN_HEADLESS=1   Same as --device-code (forces device flow)
    GRAMATR_HEADLESS=true      Same as --device-code (forces device flow)
...
`);
    ...
    // detected, OR if --device-code / --force-device-code / --headless /
    // GRAMATR_LOGIN_HEADLESS=1 / GRAMATR_HEADLESS=true is set).
    // #2372 P2-B — `--device-code` is the canonical flag name (Codex
    // precedent); `--force-device-code` and `--headless` are kept as
    // backward-compatible aliases.
    const forceHeadless = args.includes("--device-code") ||
        args.includes("--force-device-code") ||
