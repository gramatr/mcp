...
const SETTINGS_PATH = join(GRAMATR_DIR, 'settings.json');
...
function errorPage(title, detail) {
    ...
    <h2>${title}</h2>
    <p>${detail}</p>
    <div class="hint">Return to your terminal and try again, or run <code>gramatr-mcp login --token</code> to paste a token directly.</div>
  `);
}
// ── Headless Detection ──
...
export function writeConfig(config) {
    ...
    if (typeof config.user_id === 'string' && config.user_id) {
        syncUserIdToSettings(config.user_id);
    }
}
function syncUserIdToSettings(userId) {
    try {
        mkdirSync(GRAMATR_DIR, { recursive: true });
        let existing = {};
        try {
            existing = JSON.parse(readFileSync(SETTINGS_PATH, 'utf8'));
        }
        catch { /* first write */ }
        existing.auth = { ...(existing.auth ?? {}), user_id: userId };
        writeFileSync(SETTINGS_PATH, JSON.stringify(existing, null, 2) + '\n');
    }
    catch {
        // non-fatal — ~/.gramatr.json is still the canonical fallback
    }
}
...
export async function showStatus() {
    ...
    if (!token) {
        ...
        console.log('\n  Run: gramatr-mcp login to authenticate\n');
        return;
    }
    ...
    if (result.valid) {
        ...
        console.log('\n  Run: gramatr-mcp login to re-authenticate\n');
    }
    console.log('');
}
...
export async function loginBrowser(opts = {}) {
    ...
    if (isHeadless(opts.forceHeadless)) {
        ...
        try {
            ...
            console.log('  Run `gramatr-mcp login` when you\'re ready to authenticate.\n');
            process.exit(1);
            return;
        }
    }
...
export async function main(rawArgs = process.argv.slice(2)) {
    ...
    if (args.includes('--token') || args.includes('-t')) {
        ...
        if (!token) {
            ...
            console.log('  (API keys start with gramatr_sk_)\n');
  ...
  Usage:
    gramatr-mcp login              Interactive login (browser PKCE when display available,
                                   device flow otherwise — auto-detected)
    gramatr-mcp login --headless   Force the device flow even on desktops
    gramatr-mcp login --token      Paste a token (API key or OAuth token)
    gramatr-mcp login --token <t>  Provide token directly
    gramatr-mcp login --status     Check authentication status
    gramatr-mcp login --logout     Remove stored credentials
    gramatr-mcp login --help       Show this help
