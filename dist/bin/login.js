...
export function killStaleMcpServers() {
    try {
        ...
        if (process.platform === 'win32') {
            execSync('taskkill /F /IM gramatr-mcp.exe /T 2>nul', { stdio: 'ignore' });
        }
        else {
            // pkill -TERM -f matches against the full command line; the $ anchor ensures
            // we only kill processes whose argv[1] ends in 'gramatr-mcp' (the stdio server),
            // not subcommands like 'gramatr-mcp login' which have trailing arguments.
            execSync(`pkill -TERM -f 'gramatr-mcp$' 2>/dev/null; true`, { stdio: 'ignore' });
        }
    }
    catch {
