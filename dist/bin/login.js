...
export async function loginBrowser(opts = {}) {
    ...
    // --headless / --device-code / GRAMATR_LOGIN_HEADLESS=1 / GRAMATR_HEADLESS=true
    // force this path even on desktop — escape hatch when the browser flow is
    // broken or the user prefers the device flow's out-of-band UX (Codex
    // precedent).
    const headless = isHeadless(opts.forceHeadless);
    if (headless) {
        // #2372 P2-B: silent auto-switch on detected headless host, one-line
        // info log so the user knows why PKCE was skipped.
