...
async function ensureAuthenticated() {
    // If a token is already available via env or existing token file, nothing to do.
    if (getToken())
        return;
    // No token — start the RFC 8628 device flow.
    ...
    process.stderr.write('grāmatr: Authenticated. Starting proxy...\n');
}
...
async function main() {
    // Ensure the user is authenticated before accepting any MCP traffic.
