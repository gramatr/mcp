/**
 * setup-legacy — Legacy migration: cleanup of pre-rebrand artifacts.
 *
 * Extracted from setup.ts for SRP: all legacy detection, sanitization,
 * managed-block stripping, and clean-install logic.
 */
export declare function cleanLegacyClaudeArtifacts(dryRun: boolean): void;
export declare function cleanStalePluginPermissions(dryRun: boolean): void;
export declare function runCleanInstall(dryRun: boolean): void;
//# sourceMappingURL=setup-legacy.d.ts.map