...
function removeDirectoryIfExists(path, dryRun) {
    if (!existsSync(path))
        return;
    if (dryRun) {
        process.stderr.write(`[gramatr] clean-install dry-run: would remove ${path}\n`);
        return;
    }
    rmSync(path, { recursive: true, force: true });
    process.stderr.write(`[gramatr] clean-install: removed ${path}\n`);
}
...
export function cleanLegacyClaudeArtifacts(dryRun) {
    process.stderr.write('[gramatr] clean-install: removing legacy pre-rebrand artifacts\n');
    const legacyDirs = [
...
function sanitizeLegacyMcpServersInFile(path, dryRun) {
    ...
    if (JSON.stringify(next) === JSON.stringify(current))
        return;
    if (dryRun) {
        process.stderr.write(`[gramatr] clean-install dry-run: would sanitize legacy mcpServers in ${path}\n`);
        return;
    }
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify(next, null, 2) + '\n', 'utf8');
    process.stderr.write(`[gramatr] clean-install: sanitized legacy mcpServers in ${path}\n`);
}
function sanitizeHookFileAtPath(path, dryRun, label) {
    ...
    if (JSON.stringify(next) === JSON.stringify(current))
        return;
    if (dryRun) {
        process.stderr.write(`[gramatr] clean-install dry-run: would sanitize legacy hooks in ${path}\n`);
        return;
    }
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify(next, null, 2) + '\n', 'utf8');
    process.stderr.write(`[gramatr] clean-install: sanitized legacy hooks (${label}) in ${path}\n`);
}
function stripLegacyManagedBlocks(path, dryRun) {
    ...
    if (next === current)
        return;
    if (dryRun) {
        process.stderr.write(`[gramatr] clean-install dry-run: would strip legacy managed blocks in ${path}\n`);
        return;
    }
    writeFileSync(path, next, 'utf8');
    process.stderr.write(`[gramatr] clean-install: stripped legacy managed blocks in ${path}\n`);
}
const STALE_PLUGIN_PERMISSION_PATTERN = /^mcp__claude_ai_gr_matr__/;
const PERMISSION_KEYS = ['allow', 'deny', 'ask'];
function scrubPermissionsBlock(permissions) {
    if (!permissions || typeof permissions !== 'object' || Array.isArray(permissions)) {
        return { next: permissions, removed: 0 };
    }
    const source = permissions;
    const nextPerms = { ...source };
    let removed = 0;
    for (const key of PERMISSION_KEYS) {
        ...
        const filtered = entries.filter((entry) => {
            if (typeof entry !== 'string')
                return true;
            if (STALE_PLUGIN_PERMISSION_PATTERN.test(entry)) {
                removed += 1;
                return false;
            }
            return true;
        ...
        if (filtered.length !== entries.length) {
            nextPerms[key] = filtered;
        }
    }
    return { next: nextPerms, removed };
}
function scrubStalePluginPermissionsInFile(path, dryRun) {
    if (!existsSync(path))
        return;
    let raw;
    try {
    ...
    try {
        ...
        process.stderr.write(`[gramatr] clean-install: skipping malformed JSON at ${path}\n`);
        return;
    }
    ...
    let totalRemoved = 0;
    const next = { ...parsed };
    const topResult = scrubPermissionsBlock(parsed.permissions);
    if (topResult.removed > 0) {
        next.permissions = topResult.next;
        totalRemoved += topResult.removed;
    }
    const projects = parsed.projects;
    if (projects && typeof projects === 'object' && !Array.isArray(projects)) {
        const projectsRecord = projects;
        const nextProjects = { ...projectsRecord };
        let projectsChanged = false;
        for (const [projectPath, projectValue] of Object.entries(projectsRecord)) {
            if (!projectValue || typeof projectValue !== 'object' || Array.isArray(projectValue))
                continue;
            const project = projectValue;
            const result = scrubPermissionsBlock(project.permissions);
            if (result.removed > 0) {
                nextProjects[projectPath] = { ...project, permissions: result.next };
    ...
    if (totalRemoved === 0)
        return;
    if (dryRun) {
        process.stderr.write(`[gramatr] clean-install dry-run: would scrub ${totalRemoved} stale plugin permission(s) from ${path}\n`);
        return;
    }
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify(next, null, 2) + '\n', 'utf8');
    process.stderr.write(`[gramatr] clean-install: scrubbed ${totalRemoved} stale plugin permission(s) from ${path}\n`);
}
export function cleanStalePluginPermissions(dryRun) {
    const candidates = new Set();
    candidates.add(getClaudeSettingsPath());
    candidates.add(join(HOME, '.claude', 'settings.local.json'));
    candidates.add(getClaudeConfigPath());
            ...
            if (projects && typeof projects === 'object' && !Array.isArray(projects)) {
                for (const projectPath of Object.keys(projects)) {
                    if (typeof projectPath !== 'string' || !projectPath.startsWith('/'))
                        continue;
                    candidates.add(join(projectPath, '.claude', 'settings.local.json'));
                    candidates.add(join(projectPath, '.claude', 'settings.json'));
                }
            }
        }
        catch {
            // unreadable / malformed — top-level scrub will still run
        }
    }
    for (const path of candidates) {
        scrubStalePluginPermissionsInFile(path, dryRun);
    }
}
function cleanAllInstallTargets(dryRun) {
    ...
    cleanStalePluginPermissions(dryRun);
}
    ...
    for (const shimPath of candidates) {
        ...
        try {
            ...
            if (!stale)
                continue;
            if (dryRun) {
                process.stderr.write(`[gramatr] clean-install dry-run: would remove stale shim ${shimPath}\n`);
            }
            else {
                unlinkSync(shimPath);
                process.stderr.write(`[gramatr] clean-install: removed stale shim ${shimPath}\n`);
            }
        }
        catch {
            // non-critical
...
function cleanLegacyGmtrDir(dryRun) {
    ...
    if (!existsSync(gmtrDir))
        return;
    process.stderr.write(`[gramatr] clean-install: removing legacy ~/.gmtr/ directory\n`);
    if (!dryRun) {
        try {
...
export function runCleanInstall(dryRun) {
    process.stderr.write('[gramatr] clean-install: running global legacy cleanup across known install targets\n');
