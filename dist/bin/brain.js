...
import { MANIFEST_SCHEMA, decideResume, fileExists, indexManifestByPath, readManifest, sha256Hex, writeManifestAtomic, } from './upload-manifest.js';
// CLI version stamped into manifests. Synced with @gramatr/mcp package.json.
const CLI_VERSION = '0.13.75';
...
export async function runBrain(args) {
    ...
    if (parsed.resume) {
        try {
            const prior = readManifest(parsed.resume);
            priorIndex = indexManifestByPath(prior);
            process.stderr.write(`[gramatr] resume — loaded ${prior.files.length} prior entr${prior.files.length === 1 ? 'y' : 'ies'} from ${parsed.resume}\n`);
        }
        catch (err) {
    ...
    // output manifest is a complete record of the union of runs.
    const manifest = {
    ...
    for (const filePath of filePaths) {
        ...
        if (decision.kind === 'skip-previous') {
            ...
            upsertEntry({ ...decision.entry, completed_at: new Date().toISOString() });
            flushManifest();
