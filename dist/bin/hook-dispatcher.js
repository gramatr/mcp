...
export async function runHook(name, args) {
    if (!name) {
        process.stderr.write('[gramatr] hook: missing hook name\n');
        process.stderr.write('  Usage: gramatr hook <name> [args...]\n');
        process.stderr.write('         gramatr hook --list\n');
        return 1;
    }
    ...
    // compact is a CLI utility, not an event hook — handle before registry check.
    if (name === 'compact') {
        const mod = await import('../hooks/compact.js');
        return mod.runCompactHook(args);
    }
    if (!isKnownHook(name)) {
        process.stderr.write(`[gramatr] hook: unknown hook: ${name}\n`);
        process.stderr.write(`  Known hooks: ${HOOK_NAMES.join(', ')}, compact\n`);
        return 2;
    }
    let runner;
    switch (name) {
        ...
        case 'instructions-loaded': {
            const mod = await import('../hooks/instructions-loaded.js');
            runner = mod.runInstructionsLoadedHook;
            break;
        }
        case 'rating-capture': {
        ...
        case 'subagent-route': {
            const mod = await import('../hooks/subagent-route.js');
            runner = mod.runSubagentRouteHook;
            break;
        }
        case 'task-quality-gate': {
            const mod = await import('../hooks/task-quality-gate.js');
            runner = mod.runTaskQualityGateHook;
            break;
        }
        case 'tool-tracker': {
        ...
        default: {
            ...
            process.stderr.write(`[gramatr] hook: no runner registered for ${name}\n`);
            return 2;
        }
    }
    return runner(args);
