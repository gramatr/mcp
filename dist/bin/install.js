...
// ── project-level settings sweep (v0.20.6) ────────────────────────────────
/**
 ...
 */
export function cleanProjectLevelHooks(cwd, dryRun) {
    const path = join(cwd, ".claude", "settings.json");
    if (!existsSync(path))
        return null;
    const raw = readFileOr(path, null);
    if (raw === null)
        return { path, removed: 0 };
    const settings = parseJsonOr(raw, {});
    const stripped = removeUserPromptSubmitHookFromSettings(settings);
    if (stripped.removed === 0)
        return { path, removed: 0 };
    if (!dryRun) {
        backupFile(path);
        atomicWriteFile(path, JSON.stringify(stripped.next, null, 2) + "\n", 0o600);
    }
    return { path, removed: stripped.removed };
}
/**
 ...
 */
export const PRE_AUTH_READ_PATTERNS = [
    "mcp__gramatr__aggregate_stats",
    "mcp__gramatr__fetch_memory",
    "mcp__gramatr__find_related_entities",
    "mcp__gramatr__get_*",
    "mcp__gramatr__list_*",
    "mcp__gramatr__load_*",
    "mcp__gramatr__search_*",
];
export const PRE_AUTH_ALL_PATTERN = "mcp__gramatr__*";
/**
 ...
 */
export function applyPreAuthTier(settings, tier) {
    const next = { ...settings };
    const perms = {
        ...(next.permissions ?? {}),
    };
    const existing = Array.isArray(perms.allow) ? perms.allow : [];
    const stripped = existing.filter((p) => typeof p === "string" && !p.startsWith("mcp__gramatr__"));
    let allow = stripped;
    if (tier === "read") {
        allow = [...stripped, ...PRE_AUTH_READ_PATTERNS];
    }
    else if (tier === "all") {
        allow = [...stripped, PRE_AUTH_ALL_PATTERN];
    }
    if (allow.length > 0) {
        perms.allow = allow;
        next.permissions = perms;
    }
    else if (perms.allow !== undefined) {
        delete perms.allow;
...
/**
 * Interactive prompt reader for the pre-auth tier selection. Reads a single
 * line from stdin; empty / unrecognized input defaults to "none". Exposed
 * for direct testing.
 */
async function promptPreAuthTierStdin(log) {
    log("[grāmatr] Pre-authorize grāmatr MCP tools in ~/.claude/settings.json?");
    log("This lets you avoid the per-tool permission prompt on every grāmatr MCP call.");
    log("  1) None — prompt every time (default, safest)");
    log("  2) Read tools only — search, get, list, fetch, load, find, aggregate_stats, get_metrics");
    log("  3) All grāmatr tools — full read-write surface");
    process.stderr.write("Select [1/2/3] (default 1): ");
...
export async function install(opts) {
    ...
    // v0.20.6: clean-on by default; users can opt out with --keep-legacy.
    const cleanLegacy = opts.keepLegacy !== true;
    ...
    const summary = {
        ...
        projectSettingsCleaned: null,
        preAuthTier: opts.preAuthTier ?? "none",
        backups: [],
    };
    ...
    // 2. settings.json
    if (cleanLegacy) {
        log("[grāmatr] cleaning legacy grāmatr hook entries (use --keep-legacy to disable)");
    }
    const settingsPath = claudeSettingsPath(opts.home);
    const settingsRaw = readFileOr(settingsPath, null);
    let settings = parseJsonOr(settingsRaw, {});
    ...
    settings = merged.next;
    // 2a. Pre-authorization tier — interactive prompt or CLI flag.
    // Only prompt when (a) tier not preset, (b) not non-interactive, (c) not
    // dry-run, AND (d) we have either an explicit prompt fn or a TTY stdin.
    // Otherwise default to "none" so unit tests / piped invocations never
    // block on stdin.
    let chosenTier = opts.preAuthTier ?? "none";
    const canPrompt = opts.preAuthTier === undefined &&
        opts.nonInteractive !== true &&
        !dryRun &&
        (opts.preAuthPrompt !== undefined || (process.stdin.isTTY ?? false));
    if (canPrompt) {
        try {
            chosenTier = await (opts.preAuthPrompt ?? (() => promptPreAuthTierStdin(log)))();
        }
        catch {
            chosenTier = "none";
        }
    }
    summary.preAuthTier = chosenTier;
    settings = applyPreAuthTier(settings, chosenTier);
    if (chosenTier !== "none") {
        log(`[grāmatr] pre-authorized grāmatr MCP tools (tier: ${chosenTier})`);
    }
    const desiredSettings = JSON.stringify(settings, null, 2) + "\n";
    ...
    // 4. Legacy cruft cleanup is now always on (v0.20.6), gated by keepLegacy.
    if (cleanLegacy) {
    ...
    // 5. Project-level settings sweep (v0.20.6) — strip stale grāmatr hooks
    // from `<cwd>/.claude/settings.json`. We never add hooks here. Only runs
    // when an explicit `projectCwd` is supplied (the CLI entrypoint passes
    // `process.cwd()`; tests opt in deliberately).
    if (opts.projectCwd !== undefined) {
        const projectSweep = cleanProjectLevelHooks(opts.projectCwd, dryRun);
        summary.projectSettingsCleaned = projectSweep;
        if (projectSweep && projectSweep.removed > 0) {
            log(`[grāmatr] project settings (${projectSweep.path}) → removed ${projectSweep.removed} stale grāmatr hook entr${projectSweep.removed === 1 ? "y" : "ies"}`);
        }
    }
    return summary;
}
...
export async function uninstall(opts) {
    ...
    // 5. Project-level settings sweep (v0.20.6) — same strip behaviour as install.
    // Only runs when an explicit `projectCwd` is supplied.
    if (opts.projectCwd !== undefined) {
        const projectSweep = cleanProjectLevelHooks(opts.projectCwd, dryRun);
        summary.projectSettingsCleaned = projectSweep;
        if (projectSweep && projectSweep.removed > 0) {
...
export async function runInstallCli(argv) {
    ...
    // Parse --pre-auth=<tier> if present.
    let preAuthTier;
    for (const a of argv) {
        if (a.startsWith("--pre-auth=")) {
            const v = a.slice("--pre-auth=".length);
            if (v === "none" || v === "read" || v === "all") {
                preAuthTier = v;
            }
            else {
                process.stderr.write(`[gramatr] unknown --pre-auth value: ${v} (expected none|read|all)\n`);
            }
        }
    }
    const nonInteractive = argv.includes("--non-interactive") || argv.includes("--yes") || argv.includes("-y");
    // Non-interactive runs default to no pre-auth unless explicitly set.
    if (preAuthTier === undefined && nonInteractive)
        preAuthTier = "none";
    const opts = {
        home,
