...
import { buildGeminiExtensionManifest, buildGeminiHooksFile, buildLocalMcpServerEntry, buildOpenCodeMcpServerEntry, buildOpenCodePlugin, getGeminiExtensionDir, getGeminiHooksPath, getGeminiManifestPath, getOpenCodeConfigPath, getOpenCodePluginPath, mergeMcpServerConfig, mergeOpenCodeMcpConfig, } from '../setup/targets.js';
...
function stripLegacyTopLevelGramatrEnv(content) {
    return content
        .replace(/^\s*GRAMATR_(?:DIR|URL)\s*=\s*.*\n?/gm, '')
        .replace(/\n{3,}/g, '\n\n')
        .trimEnd();
}
export function ensureCodexMcpServerConfig(configToml) {
    ...
    const base = stripLegacyTopLevelGramatrEnv(stripTomlSection(stripTomlSection(configToml.trimEnd(), 'mcp_servers.gramatr.env'), 'mcp_servers.gramatr'));
...
export function emitInstallPromptSuggestion(target) {
    ...
    process.stderr.write('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    process.stderr.write('[gramatr] Paste the prompt block above into your custom instructions / project knowledge.\n');
}
export function setupCodex(dryRun = false, showPrompts = false) {
    ...
    const managedHooks = buildCodexHooksFile(gramatrDir);
    ...
    if (dryRun) {
        process.stderr.write('[gramatr] Dry run — would write to: ' + hooksPath + '\n');
        process.stderr.write(JSON.stringify(mergedHooks, null, 2) + '\n');
        process.stderr.write('[gramatr] Dry run — would write to: ' + configPath + '\n');
        process.stderr.write(updatedToml);
        process.stderr.write('\n[gramatr] Dry run — would write to: ' + agentsPath + '\n');
    ...
    process.stderr.write(`[gramatr] Configured Codex hooks in ${hooksPath}\n`);
    process.stderr.write(`[gramatr] Configured Codex MCP + hooks in ${configPath}\n`);
    process.stderr.write(`[gramatr] Configured Codex guidance in ${agentsPath}\n`);
    if (existsSync(repoLocalHooksPath)) {
        process.stderr.write(`[gramatr] Warning: repo-local Codex hooks detected at ${repoLocalHooksPath}. Global Codex hooks are authoritative and duplicate execution can occur until the repo-local file is removed.\n`);
    }
    process.stderr.write('[gramatr] Restart Codex or start a new session to pick up the change.\n');
    if (showPrompts)
...
export function setupMcpTarget(targetName, configPath, dryRun) {
    ...
    if (dryRun) {
        process.stderr.write(`[gramatr] Dry run — would write ${targetName} config to: ${configPath}\n`);
    ...
    process.stderr.write(`[gramatr] Configured ${targetName} MCP server in ${configPath}\n`);
    process.stderr.write(`[gramatr] Restart ${targetName} to pick up the change.\n`);
}
export function setupGemini(dryRun = false, showPrompts = false) {
    ...
    if (dryRun) {
        process.stderr.write('[gramatr] Dry run — would write Gemini manifest to: ' + manifestPath + '\n');
        process.stderr.write(JSON.stringify(manifest, null, 2) + '\n');
        process.stderr.write('[gramatr] Dry run — would write Gemini hooks to: ' + hooksPath + '\n');
    ...
    process.stderr.write(`[gramatr] Configured Gemini extension manifest in ${manifestPath}\n`);
    process.stderr.write(`[gramatr] Configured Gemini hooks in ${hooksPath}\n`);
    process.stderr.write('[gramatr] Restart Gemini CLI to pick up the change.\n');
    if (showPrompts)
        emitInstallPromptSuggestion('gemini-cli');
}
export function setupOpenCode(dryRun = false, showPrompts = false) {
    ...
    const current = readJsonFile(configPath, {});
    const next = mergeOpenCodeMcpConfig(current, mcpEntry);
    const pluginSource = buildOpenCodePlugin();
    const pluginPath = getOpenCodePluginPath(HOME);
    if (dryRun) {
        process.stderr.write('[gramatr] Dry run — would write OpenCode config to: ' + configPath + '\n');
        process.stderr.write(JSON.stringify(next, null, 2) + '\n');
        process.stderr.write(`[gramatr] Dry run — would write OpenCode plugin to: ${pluginPath}\n`);
        process.stderr.write(pluginSource.split('\n').slice(0, 10).join('\n') + '\n');
        if (showPrompts)
    ...
    process.stderr.write(`[gramatr] Configured OpenCode MCP server in ${configPath}\n`);
    process.stderr.write(`[gramatr] Deployed OpenCode plugin to ${pluginPath}\n`);
    process.stderr.write('[gramatr] Restart OpenCode to pick up the change.\n');
    if (showPrompts)
        emitInstallPromptSuggestion('opencode');
}
export async function setupWeb(target = 'claude-web') {
    ...
    process.stderr.write(`[gramatr] Checking server reachability at ${gramatrUrl}...\n`);
    const reachability = await validateServerReachability(gramatrUrl);
    if (reachability.reachable) {
        process.stderr.write(`[gramatr] Server is reachable (HTTP ${reachability.statusCode})\n\n`);
    }
    else {
        process.stderr.write(`[gramatr] Warning: server unreachable — ${reachability.error}\n`);
        process.stderr.write('[gramatr] Setup instructions generated anyway. Verify connectivity before use.\n\n');
    }
