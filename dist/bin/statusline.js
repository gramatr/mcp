...
export async function runStatusline(args) {
    ...
    try {
        ...
        if (!res.ok) {
            // 401/403 means the JWT has expired or the session is no longer valid.
            // Surface an actionable prompt so the user knows to re-login.
            if (res.status === 401 || res.status === 403) {
                process.stdout.write('⚠ gramatr: session expired — run /gramatr-login');
            }
            // All other non-ok statuses (500, 503, etc.) are transient — stay silent.
            return;
        }
