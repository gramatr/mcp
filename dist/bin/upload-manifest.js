 ...
 */
export const MANIFEST_SCHEMA = 'gmtr.upload-manifest.v2';
export const MANIFEST_SCHEMA_V1 = 'gmtr.upload-manifest.v1';
// ── Errors ──────────────────────────────────────────────────────────────────
/**
...
export function parseManifest(value) {
    ...
    const obj = value;
    if (obj.schema !== MANIFEST_SCHEMA && obj.schema !== MANIFEST_SCHEMA_V1) {
        throw new ManifestSchemaError(`manifest.schema must be "${MANIFEST_SCHEMA}" or "${MANIFEST_SCHEMA_V1}", got ${JSON.stringify(obj.schema)}`);
    }
    // v2 requires top-level batch_id. v1 may omit it (we backfill on read).
    if (obj.schema === MANIFEST_SCHEMA && typeof obj.batch_id !== 'string') {
        throw new ManifestSchemaError('manifest.batch_id must be a string (required in v2)');
    }
...
export function readManifest(path) {
    ...
    // v1 backwards-compat: mint a fresh batch_id so the rest of the pipeline can
    // assume it exists. WARN so the caller knows the identity differs from the
    // original run.
    if (m.schema === MANIFEST_SCHEMA_V1 && !m.batch_id) {
        const newId = randomUUID();
        process.stderr.write(`[gramatr] ⚠ manifest at ${path} is v1 (no batch_id); assigning fresh batch_id ${newId} — resumed batch identity differs from the original\n`);
        return { ...m, batch_id: newId };
    }
    return m;
}
// ── Resume index ────────────────────────────────────────────────────────────
