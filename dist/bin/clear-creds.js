...
function showHelp() {
    log(`gramatr-mcp clear-creds — Remove every stored gramatr credential

Usage:
  gramatr-mcp clear-creds`);
}
...
export function main(argv = process.argv.slice(2)) {
    ...
    if (r.strippedLegacyKey)
        log(`Stripped auth.api_key from ${legacySettingsPath()}`);
    ...
    log('');
    log("All credentials cleared. Next 'gramatr-mcp login' will re-authenticate.");
    return 0;
}
