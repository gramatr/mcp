...
export async function runAgentVerifyHook(_args = []) {
    ...
    try {
        ...
        if (record) {
            ...
            process.stderr.write(`[gramatr] Sub-agent completed${detail}. Verify ISC acceptance criteria before proceeding.\n`);
        }
        else {
            process.stderr.write('[gramatr] Sub-agent completed. No ISC scaffold found — verify results manually.\n');
        }
    }
    catch {
