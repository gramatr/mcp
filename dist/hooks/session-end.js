...
export async function runSessionEndHook(_args = []) {
    try {
        ...
        appendLine(join(home, '.claude', 'session-history.log'), `Session ended at ${timestamp} (reason: ${reason}, session: ${sessionId})`);
        if (config) {
        ...
        const stateDir = join(process.env.GRAMATR_DIR || join(home, '.gramatr'), '.state');
        let toolCallCount = 0;
