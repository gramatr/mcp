...
export async function runSessionEndHook(_args = []) {
    ...
    try {
        ...
        // This is the fix for #1105 — gramatr_session_end only saves to the
        // interaction record; gramatr_load_handoff reads from the handoff table,
        ...
        try {
            ...
            const whatsNext = recentPrompts.length > 0
                ? `Last prompts (pick up from here):\n${recentPrompts.join('\n')}`
            ...
            const dontForget = `Session ID: ${sessionId} | Project: ${projectId}`;
            await saveHandoff({
                projectId,
                sessionId,
                whereWeAre,
            ...
            // Non-critical — session end should never fail because handoff save failed
        }
        return 0;
    }
    catch (err) {
