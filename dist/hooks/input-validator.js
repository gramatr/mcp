...
import { getLatestClassification } from './lib/hook-state.js';
...
/**
 * #2658 — advisory phase-gate logging for the new enrichment tool family.
 * Emits a stderr advisory (never blocks) when an agent calls a phase-gated
 * tool outside its expected phase. The advisory is purely telemetry —
 * agents see no permission decision change.
 *
 * - get_reverse_engineering → expected during THINK
 * - get_quality_gates / add_quality_gate_criteria → expected during PLAN/VERIFY
 * - get_composed_agent → expected during PLAN/BUILD
 */
const NEW_TOOL_PHASE_HINTS = {
    get_reverse_engineering: new Set(['think', 'plan']),
    get_quality_gates: new Set(['plan', 'verify', 'build']),
    add_quality_gate_criteria: new Set(['plan', 'verify']),
    get_composed_agent: new Set(['plan', 'build']),
};
function logPhaseAdvisory(toolName, sessionId) {
    ...
    try {
        ...
        // Heuristic phase derivation: the packet carries phase_template[]; the
        // classifier record exposes only `effort`. For trivial efforts (instant,
        // fast) any enrichment call is unexpected; for higher efforts, we don't
        // know the current phase from this signal alone — only log when the
        // effort itself rules the call out.
        ...
        // For higher efforts, only flag when the effort marker is absent (no
        // packet at all → likely degraded / pre-classification call).
        if (!effort) {
...
export async function runInputValidatorHook(_args = []) {
    ...
    try {
        ...
        // Step 0: phase-advisory telemetry for the #2658 enrichment tool family.
        // Logs to stderr only — never blocks.
        try {
            logPhaseAdvisory(input.tool_name, input.session_id);
        }
        catch {
            // never block on advisory
        }
