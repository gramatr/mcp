...
export const HOOK_MANIFEST = [
    ...
    { name: 'agent-gate', description: "Deny Agent launches that lack a fresh gramatr_route_request classification.", event: 'PreToolUse', execution: 'local', status: 'active' },
    { name: 'subagent-route', description: "Before a sub-agent launches: call gramatr_route_request for the sub-task prompt, verify a fresh classification exists, and inject the Quality Gate scaffold as context for the sub-agent.", event: 'SubagentStart', execution: 'local', status: 'active' },
    ...
    { name: 'instructions-loaded', description: "When CLAUDE.md is loaded: call gramatr_resolve_project to identify or create the project, call gramatr_session_start to resume or create a session, and call gramatr_load_handoff to restore the last session state. Injects handoff context into the conversation automatically.", event: 'InstructionsLoaded', execution: 'local', status: 'active' },
];
