...
import { detectInstallLayout } from "./lib/install-detect.js";
import { startPeriodicSync, syncProjectsFromServer } from "./lib/sync-projects.js";
import { VERSION } from "./lib/version.js";
import { autoUpgrade, formatUpgradeHint, runVersionCheckAndNotify } from "./lib/version-check.js";
...
async function checkIdentityAndNotify(serverHealthy) {
    ...
    if (!userId) {
        ...
        log("   Run `gramatr login` to restore your personalized AI context.");
        log("   Memory context is disabled until you re-authenticate.");
        emitStdout("> **grāmatr:** Identity not established — run `gramatr login` to restore your personalized AI context. Memory and context are disabled for this session.");
        return;
    }
    ...
    try {
        ...
        if (data.reauth_required === true) {
            ...
            log("   Run `gramatr login` to continue — your access credentials need to be refreshed.");
            emitStdout("> **grāmatr:** Re-authentication required — run `gramatr login` before continuing. Your session will have full context once you re-authenticate.");
        }
    }
    catch {
...
export async function runSessionStartHook(_args = []) {
    ...
    try {
        ...
        if (git) {
            ...
            // #2907 — cache canonical grāmatr identity into ~/.gramatr.json so the
            // UserPromptSubmit hook can inject an Identity block louder than the
            // client-platform email Claude Code (and others) bake into CLAUDE.md.
            // Best-effort: write failures are absorbed by the helper. We preserve
            // any existing `user.name` written during onboarding.
            if (sessionResult.user) {
                try {
                    const { writeCachedUserIdentity } = await import('../user-config.js');
                    writeCachedUserIdentity({
                        id: sessionResult.user.id ?? null,
                        email: sessionResult.user.email ?? null,
                        display_name: sessionResult.user.display_name ?? null,
                        system_roles: sessionResult.user.system_roles ?? [],
                        org_memberships: sessionResult.user.org_memberships ?? [],
                        team_memberships: sessionResult.user.team_memberships ?? [],
                    });
                }
                catch {
                    // Non-critical — identity cache miss falls back to the cached
                    // value from a previous session (or to no Identity block).
                }
            }
            ...
            // #2658 — surface share_scope when the session is visible beyond the
            // owning user. The server resolves scope via resolveShareScope; we
            // display it both on stderr (visible to user) and inject as context
            // so the assistant knows to treat the session as shared.
            if (sessionResult.shareScope && sessionResult.shareScope !== 'user') {
                const scopeLabel = sessionResult.shareScope === 'team'
                    ? 'team-shared session'
                    : sessionResult.shareScope === 'org'
                        ? 'org-shared session'
                        : `${sessionResult.shareScope}-shared session`;
                log(`  Share scope: ${scopeLabel} — prompts and memory writes are visible to ${sessionResult.shareScope} members.`);
                emitStdout(`> **grāmatr:** This is a ${scopeLabel}. Anything you write here is visible to other ${sessionResult.shareScope} members.`);
            }
            ...
            if (hasRestoreContext && prepared.latestCompact) {
                log("Context Restored from /gramatr-compact");
                log("");
        ...
        // Opportunistic version check + layout-aware upgrade.
        // 1. Detect install layout (npm-global vs direct vs unknown).
        // 2. Check npm registry for newer @gramatr/mcp.
        // 3a. npm-global / unknown: never write files — surface a hint instead.
        // 3b. direct (~/.gramatr/): auto-install in background, then emit success.
        try {
            const installLayout = detectInstallLayout();
            const versionResult = await runVersionCheckAndNotify(VERSION);
            if (versionResult?.isOutdated) {
                const upgrade = autoUpgrade(versionResult.latestVersion, { layout: installLayout });
                if (upgrade.triggered) {
                    ...
                    emitStdout(`gramatr auto-upgrade in progress: v${versionResult.installedVersion} → v${versionResult.latestVersion}. ` +
                        ...
                        `If gramatr --version still shows the old version after restarting, run: hash -r (bash) or rehash (zsh) — this clears the shell's command path cache."`);
                }
                else if (installLayout === "npm-global" || installLayout === "unknown") {
                    // npm-global / unknown: file writes would be a no-op on the live PATH binary.
                    // Surface a clear hint without claiming an upgrade is in progress.
                    emitStdout(formatUpgradeHint(versionResult.installedVersion, versionResult.latestVersion));
                }
                else {
                    // direct layout but upgrade failed for another reason (lock, spawn error, etc.)
