...
import { buildGitProjectContextPayload, buildNonGitProjectContextPayload, getCurrentProjectContextPath, getLastSessionCommitsPath, normalizeSessionStartResponse, persistSessionRegistration, prepareProjectSessionState, startRemoteSession, writeCurrentProjectContextFile, } from './lib/session.js';
...
function emitClaudeContext(opts) {
    ...
    // Recent commits file
    const lastCommitsFile = getLastSessionCommitsPath();
...
export async function runSessionStartHook(_args = []) {
    try {
        ...
        if (git) {
            ...
            log(`  Project context saved to ${getCurrentProjectContextPath()}`);
            log('');
