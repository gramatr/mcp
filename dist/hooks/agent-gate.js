...
export function checkClassificationFreshness(sessionId, nowMs = Date.now()) {
    ...
    // The task assignment was set by the daemon IPC (not user-writable from MCP tools),
    // so bypassing reclassification here is safe.
    ...
    // Degraded mode — server was unavailable when this prompt was classified.
    // Allow through with a warning rather than hard-blocking sub-agent launch.
