 ...
 *
 * Emits a strong suggestion when no fresh route_request classification
 ...
 * - No SQLite record + fresh SQLite packet → allow silently
 *   (UserPromptSubmit hook stored the packet this turn via pull-model delivery)
...
import { getLatestClassification, getLatestPacketAgeMs, getSessionContext, getSessionContextByProject, } from "./lib/hook-state.js";
// ── Stdin ──
...
const WARN_REASON = "[gramatr] No fresh route_request found — sub-agent launching with context from last injected packet. " +
    "For best results, call route_request first to get a fresh RE, Quality Gate scaffold for " +
...
const SPECIFICATION_EFFORTS = new Set([
    "standard",
    "extended",
    "advanced",
    "deep",
 ...
 * @param packetAgeMs   - age of most recent packet in ms (injectable for tests; omit to read from SQLite)
 */
export function checkClassificationFreshness(sessionId, nowMs = Date.now(), packetAgeMs) {
    ...
    if (!record || !record.recorded_at) {
        // Fallback: check whether a packet was stored in SQLite this turn (pull-model delivery).
        const age = packetAgeMs ?? getLatestPacketAgeMs(sessionId, nowMs);
...
export async function runAgentGateHook(_args = []) {
    ...
    try {
        ...
        const result = checkClassificationFreshness(interactionId, nowMs);
