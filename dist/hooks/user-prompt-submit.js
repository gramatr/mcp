...
import { readCachedUserIdentity, renderIdentityBlock } from "../user-config.js";
...
/**
 * #2907 — build the canonical Identity prefix injected at the top of every
 * UserPromptSubmit additionalContext payload. The block is rendered VISUALLY
 * LOUDLY because the client platform (Claude Code / Cursor / ChatGPT) injects
 * its own subscription email into CLAUDE.md, and the agent must not default
 * to that as the grāmatr identity.
 *
 * Returns '' when no useful identity is cached — degrades silently rather
 * than emitting an empty header.
 */
function buildIdentityPrefix() {
    try {
        const identity = readCachedUserIdentity();
        if (!identity)
            return '';
        const block = renderIdentityBlock(identity, null);
        if (!block)
            return '';
        return `## Identity\n${block}\n\n`;
    }
...
export async function runUserPromptSubmitHook(_args = []) {
    ...
    try {
        ...
        // #2907 — Identity block is the loudest, first thing the agent sees on
        // every turn. Built once here and prepended to every output path below
        // so degraded / partial / full responses all carry it.
            ...
            if (lastFailure) {
                ...
                process.stdout.write(JSON.stringify({
                    hookSpecificOutput: {
                        ...
                        additionalContext: identityPrefix + ctxHint + formatFailureWarning(lastFailure) + degradedNotes,
                    },
                }));
            }
            else if (additionalContextParts.length > 0 || identityPrefix) {
                process.stdout.write(JSON.stringify({
                    hookSpecificOutput: {
                        hookEventName: "UserPromptSubmit",
                        additionalContext: identityPrefix + additionalContextParts.join('\n'),
                    },
                }));
            }
            else {
        ...
        process.stdout.write(JSON.stringify({
            hookSpecificOutput: {
                hookEventName: "UserPromptSubmit",
                additionalContext: identityPrefix + daemonWarning + ctxHintPrefix + intelligenceContext,
            },
        }));
        return 0;
    }
