...
import { appendOpHistory, appendTurn, enqueueOutboxMutation, getAllTimeTokensSaved, getCachedDirective, getSessionContext, getSessionTurnCount, hydrateSessionContextFromServer, isFilesystemAvailable, setCachedDirective, setLatestClassification, } from "./lib/hook-state.js";
import { writeCompact } from "./lib/compact-writer.js";
import { emitStatus, formatFailureWarning, formatIntelligence, savePacketFull, } from "./lib/intelligence.js";
...
export async function runUserPromptSubmitHook(_args = []) {
    ...
    try {
        ...
        try {
            ...
            const atReminder = pastThreshold && turnCount > COMPACT_TURN_THRESHOLD
            ...
            if (turnCount > 0 && turnCount % 10 === 0) {
                ...
                if (sessionCtx?.entity_id && sessionCtx.project_id) {
                    try {
                        enqueueOutboxMutation('gramatr_session_end', {
                            session_id: gramatrSessionId,
        ...
        // Save full packet locally keyed by interaction_id for pull-based access
        const interactionId = result.manifest
            ? result.manifest.interaction_id
            : undefined;
        if (interactionId) {
            const sessionCtx = getSessionContext(gramatrSessionId);
            const cl = result.classification;
            savePacketFull(result, interactionId, gramatrSessionId, sessionCtx?.project_id ?? projectId ?? null, cl?.effort_level ?? null, cl?.intent_type ?? null);
        }
        process.stdout.write(JSON.stringify({
            hookSpecificOutput: {
                hookEventName: "UserPromptSubmit",
