import { getClaudeModelFromEnv, getCompactTurnThresholdFromEnv, getGramatrTimeoutFromEnv, isGramatrEnrichEnabledFromEnv, } from "../config-runtime.js";
...
import { appendOpHistory, appendTurn, getAllTimeTokensSaved, getCachedDirective, getSessionContext, getSessionTurnCount, hydrateSessionContextFromServer, isFilesystemAvailable, setCachedDirective, setLatestClassification, } from "./lib/hook-state.js";
import { buildCompactInjection, writeCompact } from "./lib/compact-writer.js";
...
const COMPACT_TURN_THRESHOLD = getCompactTurnThresholdFromEnv();
...
export async function runUserPromptSubmitHook(_args = []) {
    ...
    try {
        ...
        // Auto-compact: when session turn count hits the threshold, snapshot context
        // and inject a directive telling the agent to prompt the user to clear/restore.
        try {
            const turnCount = getSessionTurnCount(gramatrSessionId);
            if (turnCount > 0 && turnCount % COMPACT_TURN_THRESHOLD === 0) {
                ...
                const injection = buildCompactInjection({
                    compactId: compact.id,
                    turnCount: compact.turnCount,
                    turnThreshold: COMPACT_TURN_THRESHOLD,
                    supportsHookedClear: runtime.supportsHookedClear,
                    clearCommand: runtime.clearCommand,
                });
                process.stdout.write(JSON.stringify({
                    hookSpecificOutput: {
