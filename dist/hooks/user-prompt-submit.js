...
import { pullSessionContextFromLocal } from "../proxy/local-client.js";
...
export async function runUserPromptSubmitHook(_args = []) {
    ...
    try {
        ...
        try {
            ...
            if (ctxTokensUsed > 0) {
                ...
                if (pct >= compactPct && COMPACT_CFG.auto) {
                    // Auto-compact: silent background save + queue handoff, then continue to packet injection
                    ...
                    if (sessionCtx?.entity_id) {
                        enqueueOutboxMutation("add_observation", {
                            ...
                            content: `[auto-compact ctx ${pct}% (${usedK}K/${limitK}K)] ${compact.summary}`,
                        });
                    }
        ...
        process.stdout.write(JSON.stringify({
            hookSpecificOutput: {
                hookEventName: "UserPromptSubmit",
                additionalContext: ctxHintPrefix + intelligenceContext,
            },
        }));
        return 0;
    }
