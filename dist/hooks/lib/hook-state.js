...
export function isDaemonActive() {
    ...
    return existsSync(join(dir, "daemon.active"));
}
...
export function getGramatrStateDir() {
    const dir = getGramatrDirFromEnv() || join(getHomeDir(), ".gramatr");
...
function getDb() {
    ...
    // Raised from 100 → 1000: checkpoint less frequently to reduce lock contention
    // in concurrent multi-session environments. The daemon handles TRUNCATE on close.
...
/** Close and reset the DB connection. Runs a TRUNCATE WAL checkpoint first to fully merge the WAL on close. */
export function closeDb() {
    if (_db) {
        try {
            // TRUNCATE merges all WAL frames and zeros the WAL file on close,
            // leaving the DB in a clean state for the next opener.
...
/** Append one turn record. Routes through the daemon broker to avoid SQLITE_BUSY collisions. */
export async function appendTurn(turn) {
    try {
        const { dbWriteViaDaemon } = await import("../../proxy/local-client.js");
        const routed = await dbWriteViaDaemon('turns', 'insert', turn);
        if (routed)
            return;
    }
    catch {
    ...
    // Daemon unavailable — fall back to direct SQLite write (preserves behaviour
    // when the daemon isn't running, e.g. CI / test environments).
    getDb()
        .prepare(`
    INSERT INTO turns
...
/** Append one op record. Routes through the daemon broker to avoid SQLITE_BUSY collisions. */
export async function appendOpHistory(op) {
    try {
        const { dbWriteViaDaemon } = await import("../../proxy/local-client.js");
        const routed = await dbWriteViaDaemon('op_history', 'insert', op);
        if (routed)
            return;
    }
    catch {
    ...
    // Daemon unavailable — fall back to direct SQLite write.
    getDb()
        .prepare(`
...
export function getLastSessionCommits() {
    const row = getDb()
        .prepare(`SELECT commit_log FROM session_log
...
export function getSessionTurnCount(sessionId) {
    try {
        const row = getDb()
...
export function getRecentTurns(sessionId, limit = 5) {
    try {
        return getDb()
...
/** Persist a compact snapshot. Routes through the daemon broker to avoid SQLITE_BUSY collisions. */
export async function saveCompact(record) {
    const flat = {
        id: record.id,
    ...
    try {
        const { dbWriteViaDaemon } = await import("../../proxy/local-client.js");
        const routed = await dbWriteViaDaemon('compacts', 'insert_or_replace', flat);
        if (routed)
            return;
    }
    catch {
    ...
    try {
      ...
      `)
        ...
        // Non-critical — compact write failure degrades gracefully
    }
}
/** Return the most recent compact for a project, or null if none. */
export function getLatestCompactForProject(projectId) {
    try {
        const row = getDb()
            .prepare(`
...
export function getCompactById(id) {
    try {
...
// ── Async wrappers — daemon-IPC-first, direct-SQLite fallback ─────────────────
//
// The three hot read paths below prefer the daemon's in-process cache when the
// daemon is running (avoiding a SQLite file open in the hook subprocess), and
// fall back to the synchronous SQLite reader when the daemon is unavailable.
//
// `isDaemonRunning()` and `callViaDaemon()` are dynamically imported to avoid
// pulling in net/fs dependencies at module load time for hooks that never use
// the async path.
/**
 * Async version of getSessionContext — uses daemon IPC when daemon is active,
 * falls back to direct SQLite read otherwise.
 */
export async function getSessionContextAsync(sessionId) {
    try {
        const { isDaemonRunning } = await import("../../daemon/startup.js");
        if (isDaemonRunning()) {
            const { callViaDaemon } = await import("../../proxy/local-client.js");
            const { DAEMON_UNAVAILABLE } = await import("../../daemon/ipc-protocol.js");
            const result = await callViaDaemon("db/query", {
 ...
 */
export async function getLastSessionForProjectAsync(projectId) {
    try {
        const { isDaemonRunning } = await import("../../daemon/startup.js");
        if (isDaemonRunning()) {
            const { callViaDaemon } = await import("../../proxy/local-client.js");
            const { DAEMON_UNAVAILABLE } = await import("../../daemon/ipc-protocol.js");
            const result = await callViaDaemon("db/query", {
                operation: "getLastSessionForProject",
                args: { projectId },
            });
            if (result !== DAEMON_UNAVAILABLE && result !== null && result !== undefined) {
                return result;
 ...
 * active, falls back to direct SQLite read otherwise.
 */
export async function getLocalProjectByDirectoryAsync(directory) {
    try {
        const { isDaemonRunning } = await import("../../daemon/startup.js");
        if (isDaemonRunning()) {
            const { callViaDaemon } = await import("../../proxy/local-client.js");
            const { DAEMON_UNAVAILABLE } = await import("../../daemon/ipc-protocol.js");
            const result = await callViaDaemon("db/query", {
                operation: "getLocalProjectByDirectory",
                args: { directory },
            });
            if (result !== DAEMON_UNAVAILABLE && result !== null && result !== undefined) {
                return result;
...
export function savePacket(record) {
    const db = getDb();
    db.prepare(`
  ...
  `).run(p(record));
    // Prune: keep last 20 per session
    db.prepare(`
...
export function getPacket(id) {
    return (getDb().prepare("SELECT * FROM packets WHERE id = ?").get(id) ??
        null);
}
/**
 * Returns the most recent packet for a session (or null if none exists).
 * Used by the Stop hook to read `directives.required_actions` and compare
 * against actual tool calls for compliance telemetry (#2658).
 */
export function getLatestPacketForSession(sessionId) {
    return (getDb()
        .prepare("SELECT * FROM packets WHERE session_id = ? ORDER BY created_at DESC LIMIT 1")
        .get(sessionId) ?? null);
}
/** Returns age in ms of the most recent packet for a session, or undefined if none exists. */
export function getLatestPacketAgeMs(sessionId, nowMs = Date.now()) {
    try {
        const row = getDb()
            .prepare("SELECT created_at FROM packets WHERE session_id = ? ORDER BY created_at DESC LIMIT 1")
            .get(sessionId);
        if (!row)
            return undefined;
