...
let _db = null;
/**
 * Prefix all keys of a plain object with @ for bun:sqlite named parameter binding.
 * bun:sqlite requires the param prefix (@, $, :) to be present in the object key,
 * e.g. { '@session_id': val } for a query using @session_id.
 */
function p(obj) {
...
export function getSessionContext(sessionId) {
    ...
    if (sessionId && sessionId !== 'unknown') {
        const scoped = db
            .query('SELECT * FROM session_context WHERE session_id = ? ORDER BY updated_at DESC LIMIT 1')
            .get(sessionId);
        if (scoped)
            return scoped;
    }
    const row = db
        .query('SELECT * FROM session_context ORDER BY updated_at DESC LIMIT 1')
        .get();
    return row ?? null;
}
export function getSessionContextByProject(projectId) {
    const row = getDb()
        .query('SELECT * FROM session_context WHERE project_id = ? ORDER BY updated_at DESC LIMIT 1')
        .get(projectId);
    return row ?? null;
}
...
export function flushTurns(sessionId) {
    const db = getDb();
    const rows = db
        .query('SELECT * FROM turns WHERE session_id = ? ORDER BY id')
        .all(sessionId);
...
export function flushOpHistory(sessionId) {
    const db = getDb();
    const rows = db
        .query('SELECT * FROM op_history WHERE session_id = ? ORDER BY id')
        .all(sessionId);
    db.query('DELETE FROM op_history WHERE session_id = ?').run(sessionId);
    return rows;
}
...
export function getLatestClassification(sessionId) {
    const row = getDb()
        .query('SELECT * FROM latest_classification WHERE session_id = ?')
        .get(sessionId);
    if (!row)
        return null;
...
export function getLastSessionCommits() {
    const row = getDb()
        .query('SELECT commit_log FROM session_log WHERE commit_log IS NOT NULL ORDER BY id DESC LIMIT 1')
        .get();
...
export function migrateProjectId(oldProjectId, newProjectId) {
    if (oldProjectId === newProjectId)
        return 0;
    const db = getDb();
    db.query('UPDATE session_log SET project_id = ? WHERE project_id = ?').run(newProjectId, oldProjectId);
    const row = db.query('SELECT changes() as changes').get();
...
export function markSessionSynced(sessionId, syncedAt) {
    ...
    getDb()
        .query('UPDATE session_log SET synced_at = ? WHERE session_id = ?')
        .run(ts, sessionId);
}
/** Count total sessions for a project (for "Session #N" display). */
export function getProjectSessionCount(projectId) {
    const row = getDb()
        .query('SELECT COUNT(*) as cnt FROM session_log WHERE project_id = ?')
        .get(projectId);
    return row?.cnt ?? 0;
}
