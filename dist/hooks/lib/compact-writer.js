...
export function buildCompactInjection(opts) {
    ...
    if (opts.supportsHookedClear && opts.clearCommand) {
        return (`${header}\n` +
            ...
            `Mention this briefly to the user ("gramatr has saved your context. Run \`${opts.clearCommand}\` to reset — your session will restore automatically.") ` +
            `then continue with their request normally.`);
    }
    return (`${header}\n` +
        `Context snapshot saved as \`${opts.compactId}\`. ` +
        `Mention this briefly to the user ("Context limit approaching. Run \`/gmtr-restore ${opts.compactId}\` in a new session to continue exactly where we left off.") ` +
        `then continue with their request normally.`);
}
