...
export function saveLastClassification(rootDir, classification) {
...
export function markClassificationFeedbackSubmitted(rootDir, submittedAt = now()) {
    const config = readGmtrConfig(rootDir);
    if (!config?.current_session?.last_classification)
        return;
 ...
 */
export async function callMcpTool(toolName, args, timeoutMs = 10000) {
    const result = await callMcpToolDetailed(toolName, args, timeoutMs);
    return result.data;
}
export async function callMcpToolDetailed(toolName, args, timeoutMs = 10000) {
 ...
 */
export async function callMcpToolRaw(toolName, args, timeoutMs = 10000) {
    const result = await callMcpToolDetailed(toolName, args, timeoutMs);
    if (result.data !== null) {
