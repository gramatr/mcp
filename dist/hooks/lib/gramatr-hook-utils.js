...
/**
 * Resolve the user's display name from ~/.gramatr.json → user.name.
 * Returns undefined if not set — callers should treat this as "name not captured yet."
 * Pattern matches resolveAuthToken().
 */
export function resolveUserName() {
    try {
        const configPath = join(HOME, '.gramatr.json');
        const config = JSON.parse(readFileSync(configPath, 'utf8'));
        if (typeof config.user?.name === 'string' && config.user.name.length > 0) {
...
export function migrateSettingsJson() {
    ...
    const principalName = settings?.principal?.name;
    if (principalName && !(config.user?.name)) {
        if (!config.user)
            config.user = {};
        config.user.name = principalName;
        changed = true;
    }
    const principalTimezone = settings?.principal?.timezone;
    if (principalTimezone && !(config.user?.timezone)) {
        if (!config.user)
            config.user = {};
        config.user.timezone = principalTimezone;
        changed = true;
    }
    if (changed) {
        try {
