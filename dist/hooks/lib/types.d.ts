...
export interface SessionStartResponse {
    project_id?: string;
    /** Server-resolved project slug (e.g. "gramatr/gramatr"). */
    project_slug?: string;
    /** True when the server created a new project record for this session. */
    ...
    /**
     * Session share scope (#2658) — `user` (default, private), `team`, or
     * `org`. When non-user, the SessionStart hook surfaces a status-line
     * indicator so the user knows their prompts are visible to teammates.
     */
    share_scope?: string;
    shareScope?: string;
    session?: {
    ...
    /**
     * Canonical grāmatr user identity (#2907). Cached by the SessionStart
     * hook into `~/.gramatr.json` so the UserPromptSubmit hook can inject
     * an Identity block louder than the client-platform email.
     */
    user?: {
        id?: string;
        email?: string;
    ...
    /** Client-platform identity surfaced for telemetry only (#2907). */
    client_identity?: {
        platform?: string;
...
export interface MctToolCallError {
    reason: 'auth' | 'auth_expired' | 'http_error' | 'mcp_error' | 'parse_error' | 'timeout' | 'network_error' | 'unknown';
    detail: string;
    status?: number;
}
...
export interface RouteResponse {
    ...
    project_state?: {
        ...
        /** @deprecated Legacy key — packets emitted before #2243. New consumers should read qg_summary. */
        isc_summary?: {
            total?: number;
            passing?: number;
            failing?: number;
