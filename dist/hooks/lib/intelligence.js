...
import { getGramatrStateDir, savePacket } from './hook-state.js';
// Resolved lazily so tests can override GRAMATR_DIR before first use
let _stateDir = null;
function stateDir() { return _stateDir ??= getGramatrStateDir(); }
const packetsDir = () => join(stateDir(), 'packets');
const lastPacketPath = () => join(stateDir(), 'last-packet.json');
...
export function formatFailureWarning(failure) {
    ...
    return [
        ...
        `schema: gmtr.intelligence.error.v1 | reason: ${failure.reason}`,
    ].join('\n');
}
...
// Direct injection window target. Claude Code truncates hook additionalContext to a preview
// when output exceeds its internal limit (observed at ~8KB). Stay well under that.
const PACKET_TARGET_BYTES = 6_000;
// If post-trim the packet still exceeds this, save to disk and inject a mandatory read instruction.
const INJECTION_HARD_LIMIT = 7_500;
function trimPacketForInjection(data) {
    const trimmed = cloneContractPayload(data);
    // Remove raw classifier outputs — large float arrays, not actionable
    const classification = trimmed.classification;
    if (classification) {
        delete classification.head_scores;
        delete classification.head_scores;
        // behavioral_directives are surfaced in the plaintext prefix — strip from JSON body
        delete classification.behavioral_directives;
    }
    delete trimmed.head_scores;
    // Strip enrichment — RE + ISC scaffold are large and optional at this point
    delete trimmed.enrichment;
    // Trim memory search results aggressively
    ...
    if (memory?.search_results && Array.isArray(memory.search_results)) {
        memory.search_results = memory.search_results
            ...
            .map((r) => ({
            ...r,
            content: typeof r.content === 'string' ? r.content.slice(0, 120) : r.content,
            summary: typeof r.summary === 'string' ? r.summary.slice(0, 120) : r.summary,
        }));
    }
    // Strip agent_defs inline bodies — large system prompts
    const orchestration = trimmed.orchestration;
    const agents = orchestration?.agents;
    if (agents?.agent_defs && Array.isArray(agents.agent_defs)) {
        agents.agent_defs = agents.agent_defs.map((a) => {
            const { system_prompt: _sp, ...rest } = a;
            return rest;
        });
    }
    // Strip execution diagnostics and stage timing detail
    const execution = trimmed.execution;
    if (execution) {
        delete execution.diagnostics;
        const summary = execution.summary;
        if (summary)
            delete summary.stage_timing;
    }
    // Strip process phase_template (already in plaintext prefix) and quality gate detail
    const process_ = trimmed.process;
    if (process_) {
        delete process_.phase_template;
        delete process_.quality_gate_config;
    }
    return trimmed;
}
/**
 ...
 */
export function savePacketFull(packet, interactionId, sessionId, projectId, effort, intent) {
    const payloadJson = JSON.stringify(packet, null, 2);
    try {
        if (!existsSync(packetsDir()))
            mkdirSync(packetsDir(), { recursive: true });
        const sidecarPath = join(packetsDir(), `${interactionId}.json`);
        writeFileSync(sidecarPath, payloadJson, 'utf8');
        // Keep last-packet.json as a convenience alias for tools that don't have the ID
        writeFileSync(lastPacketPath(), payloadJson, 'utf8');
        try {
            savePacket({ id: interactionId, session_id: sessionId, project_id: projectId, effort, intent, created_at: Date.now(), payload: payloadJson });
        }
        catch { /* SQLite write non-critical — sidecar file is the primary path */ }
        return sidecarPath;
    }
    catch {
        // Fallback: last-packet.json only
        try {
            if (!existsSync(stateDir()))
                mkdirSync(stateDir(), { recursive: true });
            writeFileSync(lastPacketPath(), payloadJson, 'utf8');
        }
        catch { /* fully non-critical */ }
        return lastPacketPath();
    }
}
function savePacketToDisk(packet) {
    try {
        if (!existsSync(stateDir()))
            mkdirSync(stateDir(), { recursive: true });
        writeFileSync(lastPacketPath(), JSON.stringify(packet, null, 2), 'utf8');
    }
    catch { /* non-critical */ }
}
function buildReadInstruction(path) {
    return `\n⚠ Intelligence packet too large for direct injection.\nMANDATORY: Read the full packet at "${path}" with the Read tool BEFORE responding.\n`;
}
...
export function formatIntelligence(data, enrichment) {
    if (data.schema === 'gmtr.intelligence.contract.v2'
        ...
        const result = prefix + serialized;
        if (result.length > INJECTION_HARD_LIMIT) {
            savePacketToDisk(payload);
            return prefix + buildReadInstruction(lastPacketPath());
        }
        return result;
    }
        ...
        const trimmedEnvelope = {
            ...
            packet_1: {
                ...
                search_results: Array.isArray(envelope.packet_1.search_results)
                    ? envelope.packet_1.search_results.slice(0, 3).map((r) => ({
                        ...r,
                        content: typeof r.content === 'string' ? r.content.slice(0, 120) : r.content,
                    }))
                    : envelope.packet_1.search_results,
                behavioral_directives: undefined,
            },
        };
