...
export function formatFailureWarning(failure) {
    ...
    if (failure.reason === 'server_down') {
        lines.push('Auto-restart failed. Manual recovery: run `gramatr daemon start` in a terminal, then restart Claude Code.');
    }
    lines.push(failure.reason === 'server_down'
        ? 'MANDATORY: Tell the user gramatr auto-restart failed and they need to run `gramatr daemon start` manually. Do not proceed as if the packet was delivered.'
        : 'MANDATORY: Tell the user this failed before responding. Do not proceed as if the packet was delivered.', `schema: gmtr.intelligence.error.v1 | reason: ${failure.reason}`);
    return lines.join("\n");
}
...
export function formatIntelligence(data, enrichment, turnId) {
    if (data.schema === "gmtr.intelligence.contract.v2" ||
        ...
        const cl = (raw.classification ??
            raw.unified_packet?.classification);
        const proc = (raw.process ??
            raw.unified_packet?.process);
        const phaseTemplate = Array.isArray(proc?.phase_template)
            ? proc.phase_template
            : null;
        const hardGates = normalizeHardGates(raw.directives?.hard_gates ??
            (raw.unified_packet &&
                isRecord(raw.unified_packet.directives)
        ...
        // Legacy inline fallback when no turnId available (e.g. degraded / test contexts)
        const payload = data.schema === "gmtr.intelligence.contract.v2"
            ? cloneContractPayload(raw)
            : normalizeFullContractPayload(data);
        return prefix + renderContractBlock(payload);
    }
