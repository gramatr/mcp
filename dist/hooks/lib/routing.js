...
import { ROUTING_DEFAULT_TIMEOUT_MS, ENRICHMENT_WAIT_TIMEOUT_MS, ENRICHMENT_FETCH_TIMEOUT_MS, ENRICHMENT_POLL_INTERVAL_MS, ENRICHMENT_TIMEOUT_BUFFER_MS, } from '../generated/hook-timeouts.js';
...
export async function fetchEnrichment(enrichmentId, timeoutMsOrOptions = ENRICHMENT_FETCH_TIMEOUT_MS) {
    const opts = typeof timeoutMsOrOptions === 'number'
        ? { mode: 'instant', timeoutMs: timeoutMsOrOptions, waitMs: timeoutMsOrOptions, pollMs: ENRICHMENT_POLL_INTERVAL_MS }
        : {
            mode: timeoutMsOrOptions.mode ?? 'wait',
            timeoutMs: timeoutMsOrOptions.timeoutMs ?? ENRICHMENT_FETCH_TIMEOUT_MS,
            waitMs: timeoutMsOrOptions.waitMs ?? timeoutMsOrOptions.timeoutMs ?? ENRICHMENT_WAIT_TIMEOUT_MS,
            pollMs: timeoutMsOrOptions.pollMs ?? ENRICHMENT_POLL_INTERVAL_MS,
        };
    const attemptFetch = async (timeoutMs) => {
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`timeout after ${timeoutMs + ENRICHMENT_TIMEOUT_BUFFER_MS}ms`)), timeoutMs + ENRICHMENT_TIMEOUT_BUFFER_MS));
        const raw = await Promise.race([
            callTool('gramatr_get_packet_two', { enrichment_id: enrichmentId, timeout_ms: timeoutMs }),
            timeoutPromise,
        ]);
        const result = extractToolPayload(raw);
        if (!result)
            return null;
        return result;
    };
    try {
        if (opts.mode === 'instant') {
            const result = await attemptFetch(opts.timeoutMs);
        ...
        while (Date.now() < deadline) {
            ...
            if (result.status === 'ready')
                return result;
            const retryAfter = typeof result.retry_after_ms === 'number' && result.retry_after_ms > 0
...
export function persistClassificationResult(options) {
    ...
    setLatestClassification({
        ...
        memory_tier: classification?.memory_tier || null,
