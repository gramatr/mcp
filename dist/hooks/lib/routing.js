...
export async function fetchEnrichment(enrichmentId, timeoutMsOrOptions = 5000) {
    const opts = typeof timeoutMsOrOptions === 'number'
        ? { mode: 'instant', timeoutMs: timeoutMsOrOptions, waitMs: timeoutMsOrOptions, pollMs: 500 }
        : {
            mode: timeoutMsOrOptions.mode ?? 'wait',
            timeoutMs: timeoutMsOrOptions.timeoutMs ?? 5000,
            waitMs: timeoutMsOrOptions.waitMs ?? timeoutMsOrOptions.timeoutMs ?? 20000,
            pollMs: timeoutMsOrOptions.pollMs ?? 500,
        };
    const attemptFetch = async (timeoutMs) => {
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`timeout after ${timeoutMs + 1000}ms`)), timeoutMs + 1000));
        const raw = await Promise.race([
