...
export async function routePrompt(options) {
    ...
    try {
        ...
        return { route: JSON.parse(text), error: null };
    }
    catch (error) {
...
export async function fetchEnrichment(enrichmentId, timeoutMs = 5000) {
    try {
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`timeout after ${timeoutMs + 1000}ms`)), timeoutMs + 1000));
        const raw = await Promise.race([
            callTool('gramatr_get_enrichment', { enrichment_id: enrichmentId, timeout_ms: timeoutMs }),
            timeoutPromise,
        ]);
        const text = raw?.content?.[0]?.text;
        if (!text)
