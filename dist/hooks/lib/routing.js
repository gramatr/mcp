...
// Retry config for transient empty responses (#1586).
// Auth, timeout, and network errors are not retried.
...
export async function routePrompt(options) {
    ...
    const retryDelayMs = options._retryDelayMs ?? ROUTE_RETRY_DELAY_MS;
    const args = {
        ...
        // Pass installed client version so the server can compute manifest.update_available (#1869).
    ...
    async function attempt() {
        try {
            ...
            if (!route) {
                const rawText = raw
                    ?.content?.[0]?.text ?? '';
                if (rawText.includes('tool call failed') || rawText.includes('ECONNREFUSED')) {
                    return { route: null, error: { reason: 'network_error', detail: rawText } };
                }
            ...
            if (detail.includes('401') || detail.toLowerCase().includes('unauthorized')) {
                const isExpiry = detail.includes('TOKEN_EXPIRED') || detail.toLowerCase().includes('expired');
                return { route: null, error: { reason: isExpiry ? 'auth_expired' : 'auth', detail } };
            }
    ...
    for (let i = 0; i < ROUTE_MAX_RETRIES; i++) {
        if (result.error?.reason !== 'parse_error')
            break;
...
export function describeRoutingFailure(error) {
    switch (error.reason) {
        ...
        case 'auth_expired':
            return {
                title: 'Routing request failed — gramatr token has expired.',
                detail: error.detail,
                action: 'Run /mcp in Claude Code to reconnect, or run `gramatr login` in a terminal.',
            };
        case 'timeout':
            return {
        ...
        default:
            return {
                ...
                detail: error.detail,
                action: 'Inspect the route_request handler response. Check GRAMATR_URL and auth token.',
            };
    }
}
