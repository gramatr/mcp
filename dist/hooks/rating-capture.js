...
export async function runRatingCaptureHook(_args = []) {
    ...
    try {
        ...
        // #1214: enqueue the rating via the local SQLite mutation outbox so we
        // survive network blips. Best-effort drain immediately afterwards — if
        // the network is up the row is replicated in-process; if it isn't, the
        // row waits until session-start (or the next rating) retries.
        try {
            enqueueOutboxMutation("gramatr_submit_feedback", {
            ...
            // Outbox unavailable — swallow so ratings.jsonl (the human audit log)
