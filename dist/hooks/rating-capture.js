...
import { RATING_FEEDBACK_TIMEOUT_MS, RATING_API_TIMEOUT_MS } from './generated/hook-timeouts.js';
...
export async function runRatingCaptureHook(_args = []) {
    ...
    try {
        ...
        fetch(`${apiBase}/feedback`, {
            ...
            signal: AbortSignal.timeout(RATING_API_TIMEOUT_MS),
        }).catch(() => { });
        if (result.rating <= 3) {
            try {
                ...
                if (p === 'darwin') {
                    spawn('osascript', ['-e', `display notification "${msg}" with title "gramatr" subtitle "Low Rating Alert"`], { stdio: 'ignore', detached: true }).unref();
                }
                else if (p === 'linux') {
                    spawn('notify-send', ['gramatr — Low Rating', msg], { stdio: 'ignore', detached: true }).unref();
                }
                else if (p === 'win32') {
