...
export async function replayQueue() {
    ...
    try {
        ...
        process.stderr.write(`[gramatr] Replaying ${entries.length} queued call(s)...\n`);
        let replayed = 0;
        let dropped = 0;
        const remaining = [];
        for (const entry of entries) {
            try {
                ...
                if (isNetworkError(error)) {
                    ...
                    dropped++;
                    process.stderr.write(`[gramatr] Dropped ${entry.tool} after ${MAX_ATTEMPTS} attempts\n`);
                }
                else {
        ...
        process.stderr.write(`[gramatr] Replay complete: ${replayed} sent, ${dropped} dropped, ${remaining.length} remaining\n`);
